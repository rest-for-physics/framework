...
void EventAction::BeginOfEventAction(const G4Event* evt) {
  if (evt->GetEventID() % 10000 == 0)
    cout << "Starting event : " << evt->GetEventID() << endl;
...
void EventAction::EndOfEventAction(const G4Event* evt) {
  ...
  for (int subId = 0; subId < restG4Event->GetNumberOfSubEventIDTracks();
       subId++) {
    FillSubEvent(subId);
    ...
    if (subRestG4Event->GetSensitiveVolumeEnergy() > 0 && en > minEnergy &&
        en < maxEnergy) {
      if (restRun->GetAnalysisTree() != NULL)
        restRun->GetAnalysisTree()->Fill();
      if (restRun->GetEventTree() != NULL) restRun->GetEventTree()->Fill();
    }
  }
...
void EventAction::FillSubEvent(Int_t subId) {
  ...
  for (int n = 0; n < restG4Event->GetNumberOfPrimaries(); n++) {
    subRestG4Event->SetPrimaryEventParticleName(
        restG4Event->GetPrimaryEventParticleName(n));
    subRestG4Event->SetPrimaryEventDirection(
        restG4Event->GetPrimaryEventDirection(n));
    subRestG4Event->SetPrimaryEventEnergy(
        restG4Event->GetPrimaryEventEnergy(n));
  }
  ...
  if (restG4Metadata->isVolumeStored(restG4Metadata->GetSensitiveVolume())) {
    Int_t sensVolID =
        restG4Metadata->GetActiveVolumeID(restG4Metadata->GetSensitiveVolume());

    subRestG4Event->SetSensitiveVolumeEnergy(
        subRestG4Event->GetEnergyDepositedInVolume(sensVolID));
  }
