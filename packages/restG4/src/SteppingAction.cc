...
void SteppingAction::UserSteppingAction(const G4Step* aStep) {
  ...
  if (!aStep->GetPostStepPoint()->GetProcessDefinedStep()) {
    ...
    G4cout << "This is a sign of problem in the restG4 particle definition"
           << endl;
    G4cout << endl;
  ...
  if (biasing > 0) {
    ...
    if (restBiasingVolume.isInside(x, y, z) && nom_part == "gamma") {
      ...
      if (eKinetic > restBiasingVolume.GetMinEnergy() &&
          eKinetic < restBiasingVolume.GetMaxEnergy()) {
        G4ThreeVector position = aStep->GetPreStepPoint()->GetPosition();
        G4ThreeVector positionNorm =
            -aStep->GetPreStepPoint()->GetPosition().unit();
    ...
    for (int volID = 0; volID < restG4Metadata->GetNumberOfActiveVolumes();
         volID++) {
      ...
      if (restG4Event->isVolumeStored(volID) || isDecay) {
        if (restG4Metadata->GetVerboseLevel() >= REST_Extreme)
          G4cout << "Step volume :" << nom_vol << "::("
                 << (G4String)restG4Metadata->GetActiveVolumeName(volID) << ")"
                 << G4endl;

        // We store the hit if we have activated in the config
        Bool_t isActiveVolume =
            (nom_vol == (G4String)restG4Metadata->GetActiveVolumeName(volID));
