...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  auto pathandname =
      SeparatePathAndName((string)restG4Metadata->Get_GDML_Filename());
  ...
  if (type == "volume" && volume != "Not defined") {
    ...
    if (pVol == NULL) {
      cout << "ERROR : The generator volume was not found in the geometry"
           << endl;
      exit(1);
      return W;
    }
    ...
    if (type == "surface" || type == "volume") {
      restG4Metadata->SetGeneratorPosition(fGeneratorTranslation.x(),
                                           fGeneratorTranslation.y(),
  ...
  for (int id = 0; id < restG4Metadata->GetNumberOfActiveVolumes(); id++) {
    G4VPhysicalVolume* pVol =
        GetPhysicalVolume((G4String)restG4Metadata->GetActiveVolumeName(id));

    cout << "Activating volume : " << restG4Metadata->GetActiveVolumeName(id)
         << endl;
...
//_____________________________________________________________________________
G4VPhysicalVolume* DetectorConstruction::GetPhysicalVolume(
    G4String physVolName) {
  G4PhysicalVolumeStore* physVolStore = G4PhysicalVolumeStore::GetInstance();
  std::vector<G4VPhysicalVolume*>::const_iterator physVol;
  for (physVol = physVolStore->begin(); physVol != physVolStore->end();
       physVol++) {
