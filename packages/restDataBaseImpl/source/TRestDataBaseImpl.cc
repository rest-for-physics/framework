...
int TRestDataBaseImpl::query_run(int runnumber, int subrun) {
  auto a = query(conn, "select run_id from rest_runs where run_id=$1::int",
                 runnumber);
...
DataBaseFileInfo TRestDataBaseImpl::query_fileinfo(int runnumber, int subrun,
                                                   string filename) {
  string querystring;
  if (filename != "") {
    querystring =
...
string TRestDataBaseImpl::query_type(int runnumber, int subrun) {
  auto a = query(
      conn,
      "select type from rest_runs where run_id=$1::int and subrun_id=$2::int",
      runnumber, subrun);
...
string TRestDataBaseImpl::query_user(int runnumber, int subrun) {
  auto a = query(
      conn,
      "select usr from rest_runs where run_id=$1::int and subrun_id=$2::int",
      runnumber, subrun);
  return a.size() > 0 ? a[0][0] : "";
}
string TRestDataBaseImpl::query_tag(int runnumber, int subrun) {
  auto a = query(
      conn,
      "select tag from rest_runs where run_id=$1::int and subrun_id=$2::int",
      runnumber, subrun);
  return a.size() > 0 ? a[0][0] : "";
}
...
vector<pair<int, int>> TRestDataBaseImpl::search_filepattern(
    string filepattern) {
  ...
  auto a = query(
      conn,
      "select run_id, subrun_id from rest_files where file_name like $1::text",
      namepattern);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
        pair<int, int>(StringToInteger(a[i][0]), StringToInteger(a[i][1])));
  }
  return result;
}
vector<pair<int, int>> TRestDataBaseImpl::search_withintime(time_t t1,
                                                            time_t t2) {
  auto a = query(conn,
                 "select run_id, subrun_id from rest_runs where "
                 "run_start>=to_timestamp($1::bigint) and "
                 "run_end<=to_timestamp($2::bigint)",
                 t1, t2);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
...
vector<pair<int, int>> TRestDataBaseImpl::search_tag(string tag) {
  ...
  _tag = Replace(_tag, "?", "%", 0);
  auto a = query(
      conn, "select run_id, subrun_id from rest_runs where tag like $1::text",
      _tag);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
...
vector<pair<int, int>> TRestDataBaseImpl::search_user(string user) {
  ...
  auto a = query(
      conn, "select run_id, subrun_id from rest_runs where usr like $1::text",
      _user);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
...
vector<pair<int, int>> TRestDataBaseImpl::search_type(string type) {
  ...
  auto a = query(conn,
                 "select run_id, subrun_id from rest_runs where cast(type as "
                 "text) like $1::text",
                 _type);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
...
vector<pair<int, int>> TRestDataBaseImpl::search_description(
    string description) {
  string _description = Replace(description, "*", "%", 0);
  _description = Replace(_description, "?", "%", 0);
  auto a = query(
      conn,
      "select run_id, subrun_id from rest_runs where description like $1::text",
      _description);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
...
vector<pair<int, int>> TRestDataBaseImpl::search_version(string version) {
  ...
  auto a = query(
      conn,
      "select run_id, subrun_id from rest_runs where version like $1::text",
      _version);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
...
vector<pair<int, int>> TRestDataBaseImpl::searchexp(string expresstion) {
  auto a = query(
      conn, "select run_id, subrun_id from rest_runs where " + expresstion);
  vector<pair<int, int>> result(0);
  for (int i = 0; i < a.size(); i++) {
    result.push_back(
        pair<int, int>(StringToInteger(a[i][0]), StringToInteger(a[i][1])));
...
pair<int, int> TRestDataBaseImpl::getrunwithfilename(string filename) {
  ...
  if (a.size() == 0) {
    cout << "REST_WARNING : find zero runs with file \"" << filename << "\"!"
         << endl;
    return pair<int, int>(0, 0);
  }
  if (a.size() > 1) {
    cout << "REST_WARNING : find multiple runs with file \"" << filename
         << "\", returning the last one" << endl;
  }
  return a[a.size() - 1];
}
...
int TRestDataBaseImpl::getlastsubrun(int runnumber) {
  auto a =
      query(conn, "select MAX(subrun_id) from rest_runs where run_id=$1::int",
            runnumber);
  return StringToInteger(a[0][0]);
}
...
int TRestDataBaseImpl::new_run() {
  ...
  auto a = query(conn,
                 ...
                 fRunNumber, fSubRunNumber, user, version);
...
int TRestDataBaseImpl::new_run(int runnumber) {
  ...
  if (runnumber > last_run + 1) {
    cout << "REST ERROR : cannot create run with run id incoherent! (last run: "
         << last_run << ", given run: " << runnumber << ")" << endl;
...
int TRestDataBaseImpl::new_runfile(string filename, DataBaseFileInfo info) {
  ...
  if (fRunNumber >= 0 && fSubRunNumber >= 0) {
    ...
    auto a =
        query(conn,
              "insert into rest_files(run_id, subrun_id, file_id, file_name, "
              "file_size, start_time, stop_time, event_rate, sha1sum, quality "
              ") values($1::int, $2::int, $3::int, $4::text, $5::bigint, "
              "$6::timestamp with time zone, $7::timestamp with time zone, "
              "$8::real, $9::text, $10::bool)",
              fRunNumber, fSubRunNumber, nFiles, ToAbsoluteName(filename),
              ToString(info.fileSize), ToDateTimeString(info.start),
              ToDateTimeString(info.stop), info.evtRate, (string)info.sha1sum,
              info.quality);
  } else {
...
int TRestDataBaseImpl::set_fileinfo(int fileid, DataBaseFileInfo info) {
  query(conn,
        ...
        "run_id=$1::int and subrun_id=$2::int and file_id=$3::int",
        fRunNumber, fSubRunNumber, fileid, ToString(info.fileSize),
...
int TRestDataBaseImpl::set_fileinfo(string filename, DataBaseFileInfo info) {
  query(conn,
        ...
        fRunNumber, fSubRunNumber, ToAbsoluteName(filename),
        ToString(info.fileSize), ToDateTimeString(info.start),
        ToDateTimeString(info.stop), info.evtRate, (string)info.sha1sum,
        info.quality);
  return 0;
...
void TRestDataBaseImpl::print(int runnumber, int subrun) {
  ...
  if (a.size() == 1) {
    ...
    cout << "-------------------" << endl;
    cout << "Time: " << ToDateTimeString(ToTime(a[0][1])) << " to "
         << ToDateTimeString(ToTime(a[0][2])) << endl;
