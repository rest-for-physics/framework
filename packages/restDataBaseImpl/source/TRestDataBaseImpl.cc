...
vector<pair<int, int>> TRestDataBaseImpl::search_filepattern(string filepattern) {
    ...
    auto a =
        query(conn, "select run_id, subrun_id from rest_files where file_name like $1::text", namepattern);
...
vector<pair<int, int>> TRestDataBaseImpl::search_description(string description) {
    ...
    auto a =
        query(conn, "select run_id, subrun_id from rest_runs where description like $1::text", _description);
    vector<pair<int, int>> result(0);
    for (int i = 0; i < a.size(); i++) {
        result.push_back(pair<int, int>(StringToInteger(a[i][0]), StringToInteger(a[i][1])));
    }
...
pair<int, int> TRestDataBaseImpl::getrunwithfilename(string filename) {
    ...
    if (a.size() > 1) {
        cout << "REST_WARNING : find multiple runs with file \"" << filename << "\", returning the last one"
             << endl;
    }
    return a[a.size() - 1];
}
...
int TRestDataBaseImpl::set_fileinfo(int fileid, DataBaseFileInfo info) {
    query(conn,
          ...
          fRunNumber, fSubRunNumber, fileid, ToString(info.fileSize), ToDateTimeString(info.start),
          ToDateTimeString(info.stop), info.evtRate, (string)info.sha1sum, info.quality);
...
void TRestDataBaseImpl::print(int runnumber, int subrun) {
    ...
    if (a.size() == 1) {
        ...
        cout << "-------------------" << endl;
        cout << "Time: " << ToDateTimeString(ToTime(a[0][1])) << " to " << ToDateTimeString(ToTime(a[0][2]))
             << endl;
