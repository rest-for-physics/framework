...
Int_t REST_CheckReadout( TString rootFile, TString name, Double_t region[4], Int_t stripsMask[4], Int_t N = 1E4, Int_t plane = 0 )
{
...
TGraph *GetHittedStripMap( TRestReadoutPlane *p, Int_t mask[4], Double_t region[4], Int_t N )
{
    ...
    while( N > 0 )
    {
        ...
        for( int mod = 0; mod < p->GetNumberOfModules(); mod++ )
        {
            ...
            if( chFound != -1 && std::find(channelIds.begin(), channelIds.end(), chFound ) != channelIds.end())
            {
                xPos.push_back( xR );
                yPos.push_back( yR );
            }
        ...
        if( N%10000 == 0 ) { system( "date" ); cout << "N: " << N << endl; }
    }
