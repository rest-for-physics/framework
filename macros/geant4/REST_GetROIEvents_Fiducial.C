...
Double_t REST_Fiducial( TString fName, Double_t zMin, Double_t zMax, Double_t radius, Double_t mean=2457.83, Double_t fwhm=25 )
{
           ...
           for( int k = 0; k < ev->GetTrack( j )->GetNumberOfHits(); k++ )
           {
               if( ev->GetTrack( j )->GetHits()->GetEnergy( k ) > 0 )
               {
                   ...
                   if( z > zMin && z < zMax )
                   {
                       ...
                       if( r < radius )
                           eDep +=  ev->GetTrack( j )->GetHits()->GetEnergy( k );
