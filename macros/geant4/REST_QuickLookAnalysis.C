...
Int_t REST_QuickLookAnalysis(TString fName) {
    ...
    TTree* tr = (TTree*)f->Get("TRestGeant4EventTree");
    ...
    TH1D* depositedEnergy_ROI_Histogram =
        new TH1D("Edep_ROI", "Total deposited energy", nbinsROI, eMin_ROI, eMax_ROI);
    ...
    TH2D* eventOriginXY_Histo =
        new TH2D("XY_Dist", "Event origin in XY (slide 20cm)", nbins, -xMax, xMax, nbins, -yMax, yMax);
    TH2D* eventOriginYZ_Histo =
        new TH2D("YZ_Dist", "Event origing in ZY (slide 20cm)", nbins, -yMax, yMax, nbins, -zMax, zMax);
    ...
    for (int evID = 0; evID < tr->GetEntries(); evID++) {
        ...
        if (eDep > 0) {
            ...
            if (eDep > eMin_ROI && eDep < eMax_ROI) depositedEnergy_ROI_Histogram->Fill(eDep);
            //       ev->PrintEvent();
