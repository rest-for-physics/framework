...
PhysicsList::PhysicsList(TRestG4PhysicsLists* physicsLists)
...
void PhysicsList::InitializePhysicsLists() {
  ...
  if (restPhysList->FindPhysicsList("G4RadioactiveDecayPhysics") >= 0)
    ...
    G4cout << "restG4. PhysicsList. G4RadioactiveDecayPhysics is not enabled!!"
           << G4endl;
  }
  ...
  if (restPhysList->GetVerboseLevel() >= REST_Essential && emCounter == 0) {
    G4cout
        << "REST WARNING : No electromagenetic physics list has been enabled!!"
        << G4endl;
  }
  ...
  G4cout << "Number of hadronic physics lists added " << fHadronPhys.size()
         << G4endl;
}
//%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
void PhysicsList::ConstructProcess() {
  ...
  if (fEmPhysicsList != NULL) {
    ...
    emOptions.SetFluo(
        true);  // To activate deexcitation processes and fluorescence
    emOptions.SetAuger(
        true);  // To activate Auger effect if deexcitation is activated
    emOptions.SetPIXE(
        true);  // To activate Particle Induced X-Ray Emission (PIXE)
  }
  ...
  if (restPhysList->FindPhysicsList("G4RadioactiveDecay")) {
    ...
    if (restPhysList->GetPhysicsListOptionValue("G4RadioactiveDecay", "ICM") ==
        "true")
      radioactiveDecay->SetICM(true);  // Internal Conversion
    else if (restPhysList->GetPhysicsListOptionValue("G4RadioactiveDecay",
                                                     "ICM") == "false")
    ...
    if (restPhysList->GetPhysicsListOptionValue("G4RadioactiveDecay", "ARM") ==
        "true")
