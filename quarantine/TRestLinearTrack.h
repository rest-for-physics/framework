...
class TRestLinearTrack : public TObject {
   ...
   public:
    ...
    Bool_t isXY() {
        if (TMath::IsNaN(fMeanPosition.Z())) return true;
        return false;
    }

    Bool_t isYZ() {
        if (TMath::IsNaN(fMeanPosition.X())) return true;
        return false;
    }

    Bool_t isXZ() {
        if (TMath::IsNaN(fMeanPosition.Y())) return true;
        return false;
    }
    ...
    Bool_t isXYZ() {
        ...
        if (!TMath::IsNaN(x) && !TMath::IsNaN(y) && !TMath::IsNaN(z)) return true;
    ...
    void AddChargeToLongitudinalTrack(Double_t lin, Double_t charge) { fLinearCharge.AddCharge(lin, charge); }
    ...
    void SortTracks() {
        fLinearCharge.Sort();
        fTransversalCharge.Sort();
    }

    TGraph* GetLinearGraph(Int_t color = 1) { return fLinearCharge.GetGraph(color); }

    TGraph* GetTransversalGraph(Int_t color = 1) { return fTransversalCharge.GetGraph(color); }
