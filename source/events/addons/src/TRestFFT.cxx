...
void TRestFFT::ForwardSignalFFT(TRestSignal* sgnl, Int_t fNStart, Int_t fNEnd) {
  ...
  for (int i = 0; i < fNfft; i++)
    forward->GetPointComplex(i, fFrequencyReal.GetArray()[i],
                             fFrequencyImg.GetArray()[i]);
...
void TRestFFT::BackwardFFT() {
  ...
  backward->SetPointsComplex(fFrequencyReal.GetArray(),
                             fFrequencyImg.GetArray());
  backward->Transform();

  for (int i = 0; i < fNfft; i++) {
    backward->GetPointComplex(i, fTimeReal.GetArray()[i],
                              fTimeImg.GetArray()[i]);
...
void TRestFFT::MultiplyBy(TRestFFT* fftInput, Int_t from, Int_t to) {
  ...
  for (int i = from; i < GetNfft() / 2; i++) {
    TComplex top(this->GetFrequencyAmplitudeReal(i),
                 this->GetFrequencyAmplitudeImg(i));
    TComplex bottom(fftInput->GetFrequencyAmplitudeReal(i),
                    fftInput->GetFrequencyAmplitudeImg(i));
...
void TRestFFT::ButterWorthFilter(
    Int_t cutOff, Int_t order)  //, Double_t amp, Double_t decay )
{
  double cOffDouble = (double)cutOff;
  for (int i = 0; i < fNfft / 2; i++) {
    double iDouble = (double)i;
    if (i > cutOff) {
      fFrequencyReal.GetArray()[i] /=
          sqrt(1 + pow(iDouble / cOffDouble, 2 * order));
      fFrequencyImg.GetArray()[i] /=
          sqrt(1 + pow(iDouble / cOffDouble, 2 * order));
...
void TRestFFT::GaussianSecondOrderResponse(Double_t f1, Double_t f2,
                                           Double_t Ao, Double_t sigma) {
...
void TRestFFT::SetSecondOrderAnalyticalResponse(Double_t f1, Double_t f2,
                                                Double_t to) {
  for (int i = 0; i < fNfft / 2; i++) {
...
void TRestFFT::RenormalizeNode(Int_t n, Double_t factor) {
  fFrequencyReal.GetArray()[fNfft - n - 1] =
      fFrequencyReal.GetArray()[n] / factor;
  fFrequencyImg.GetArray()[fNfft - n - 1] =
      fFrequencyImg.GetArray()[n] / factor;
