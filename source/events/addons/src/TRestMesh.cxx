...
Int_t TRestMesh::GetNodeX(Double_t x) {
  ...
  if (nX < 0) {
    cout << "REST WARNING (TRestMesh) : X node (" << x
         << ") outside boundaries. Setting it to : " << 0 << endl;
    return 0;
  }
...
Int_t TRestMesh::GetNodeZ(Double_t z) {
  ...
  if (nZ < 0) {
    cout << "REST WARNING : Z node outside boundaries. Setting it to : " << 0
         << endl;
    return fNodesZ - 1;
  }
  if (nZ >= fNodesZ) {
    cout << "REST WARNING : Z node outside boundaries. Setting it to : "
         << fNodesZ - 1 << endl;
    return fNodesZ - 1;
  }
  return nZ;
...
void TRestMesh::Print() {
  ...
  std::cout << "---------------------------------------------" << std::endl;
  for (int i = 0; i < GetNumberOfNodes(); i++)
    std::cout << "Group : " << nodeGroupID[i] << " X : " << nodeX[i]
              << " Y : " << nodeY[i] << " Z : " << nodeZ[i] << std::endl;
  std::cout << "---------------------------------------------" << std::endl;
}
