...
TVector3 TRestG4Event::GetMeanPositionInVolume(Int_t volID) {
  ...
  for (int t = 0; t < GetNumberOfTracks(); t++) {
    ...
    if (tck->GetEnergyInVolume(volID) > 0) {
      pos +=
          tck->GetMeanPositionInVolume(volID) * tck->GetEnergyInVolume(volID);
...
void TRestG4Event::SetBoundaries() {
  Double_t maxX = -1e10, minX = 1e10, maxZ = -1e10, minZ = 1e10, maxY = -1e10,
           minY = 1e10;
...
TMultiGraph* TRestG4Event::GetXYMultiGraph(Int_t gridElement,
                                           std::vector<TString> pcsList,
                                           Double_t minPointSize,
                                           Double_t maxPointSize) {
  if (fXYHitGraph != NULL) {
    delete[] fXYHitGraph;
    fXYHitGraph = NULL;
  }
  ...
  fPad->cd(gridElement)
      ->DrawFrame(fMinX - 10, fMinY - 10, fMaxX + 10, fMaxY + 10, title);
  ...
  for (int ntck = 0; ntck < GetNumberOfTracks(); ntck++) {
    ...
    for (int nhit = 0; nhit < hits->GetNumberOfHits(); nhit++) {
      ...
      for (unsigned int p = 0; p < pcsList.size(); p++) {
        if (GetTrack(ntck)->GetProcessName(hits->GetProcess(nhit)) ==
            pcsList[p]) {
          ...
          if (legendAdded[p] == 0) {
            fLegend_XY->AddEntry(
                pcsGraph,
                GetTrack(ntck)->GetProcessName(hits->GetProcess(nhit)), "p");
            legendAdded[p] = 1;
          }
        }
      }
  ...
  fXYMultiGraph->GetXaxis()->SetTitleSize(
      1.25 * fXYMultiGraph->GetXaxis()->GetTitleSize());
  fXYMultiGraph->GetXaxis()->SetTitleOffset(1.25);
  fXYMultiGraph->GetXaxis()->SetLabelSize(
      1.25 * fXYMultiGraph->GetXaxis()->GetLabelSize());

  fXYMultiGraph->GetYaxis()->SetTitle("Y-axis (mm)");
  fXYMultiGraph->GetYaxis()->SetTitleSize(
      1.25 * fXYMultiGraph->GetYaxis()->GetTitleSize());
  fXYMultiGraph->GetYaxis()->SetTitleOffset(1.75);
  fXYMultiGraph->GetYaxis()->SetLabelSize(
      1.25 * fXYMultiGraph->GetYaxis()->GetLabelSize());
  fPad->cd(gridElement);

  return fXYMultiGraph;
}

TMultiGraph* TRestG4Event::GetYZMultiGraph(Int_t gridElement,
                                           std::vector<TString> pcsList,
                                           Double_t minPointSize,
                                           Double_t maxPointSize) {
  if (fYZHitGraph != NULL) {
  ...
  fPad->cd(gridElement)
      ->DrawFrame(fMinY - 10, fMinZ - 10, fMaxY + 10, fMaxZ + 10, title);
  ...
  fYZMultiGraph->GetXaxis()->SetTitleSize(
      1.25 * fYZMultiGraph->GetXaxis()->GetTitleSize());
  fYZMultiGraph->GetXaxis()->SetTitleOffset(1.25);
  fYZMultiGraph->GetXaxis()->SetLabelSize(
      1.25 * fYZMultiGraph->GetXaxis()->GetLabelSize());

  fYZMultiGraph->GetYaxis()->SetTitle("Z-axis (mm)");
  fYZMultiGraph->GetYaxis()->SetTitleSize(
      1.25 * fYZMultiGraph->GetYaxis()->GetTitleSize());
  fYZMultiGraph->GetYaxis()->SetTitleOffset(1.75);
  fYZMultiGraph->GetYaxis()->SetLabelSize(
      1.25 * fYZMultiGraph->GetYaxis()->GetLabelSize());
  fPad->cd(gridElement);

  return fYZMultiGraph;
}

TMultiGraph* TRestG4Event::GetXZMultiGraph(Int_t gridElement,
                                           std::vector<TString> pcsList,
                                           Double_t minPointSize,
                                           Double_t maxPointSize) {
  if (fXZHitGraph != NULL) {
  ...
  fPad->cd(gridElement)
      ->DrawFrame(fMinX - 10, fMinZ - 10, fMaxX + 10, fMaxZ + 10, title);
  ...
  fXZMultiGraph->GetXaxis()->SetTitleSize(
      1.25 * fXZMultiGraph->GetXaxis()->GetTitleSize());
  fXZMultiGraph->GetXaxis()->SetLabelSize(
      1.25 * fXZMultiGraph->GetXaxis()->GetLabelSize());

  fXZMultiGraph->GetYaxis()->SetTitle("Z-axis (mm)");
  fXZMultiGraph->GetYaxis()->SetTitleOffset(1.75);
  fXZMultiGraph->GetYaxis()->SetTitleSize(
      1.25 * fXZMultiGraph->GetYaxis()->GetTitleSize());
  fXZMultiGraph->GetYaxis()->SetLabelSize(
      1.25 * fXZMultiGraph->GetYaxis()->GetLabelSize());
  fPad->cd(gridElement);
...
TH2F* TRestG4Event::GetXYHistogram(Int_t gridElement,
  ...
  fXYHisto = new TH2F("XY", "", nBinsX, fMinX - 10, fMinX + pitch * nBinsX,
...
TH2F* TRestG4Event::GetXZHistogram(Int_t gridElement,
                                   std::vector<TString> optList) {
  if (fXZHisto != NULL) {
    delete fXZHisto;
    fXZHisto = NULL;
  ...
  fXZHisto = new TH2F("XZ", "", nBinsX, fMinX - 10, fMinX + pitch * nBinsX,
...
TH2F* TRestG4Event::GetYZHistogram(Int_t gridElement,
                                   std::vector<TString> optList) {
  if (fYZHisto != NULL) {
    delete fYZHisto;
    fYZHisto = NULL;
  ...
  fYZHisto = new TH2F("YZ", "", nBinsY, fMinY - 10, fMinY + pitch * nBinsY,
                      nBinsZ, fMinZ - 10, fMinZ + pitch * nBinsZ);
...
TH1D* TRestG4Event::GetXHistogram(Int_t gridElement,
                                  std::vector<TString> optList) {
  if (fXHisto != NULL) {
    delete fXHisto;
    fXHisto = NULL;
...
TH1D* TRestG4Event::GetZHistogram(Int_t gridElement,
                                  std::vector<TString> optList) {
  if (fZHisto != NULL) {
    delete fZHisto;
    fZHisto = NULL;
...
TH1D* TRestG4Event::GetYHistogram(Int_t gridElement,
                                  std::vector<TString> optList) {
  if (fYHisto != NULL) {
    delete fYHisto;
    fYHisto = NULL;
...
TPad* TRestG4Event::DrawEvent(TString option) {
  ...
  optList.erase(std::remove(optList.begin(), optList.end(), "print"),
                optList.end());
...
void TRestG4Event::PrintEvent(int maxTracks, int maxHits) {
  ...
  cout << "Sensitive volume energy : " << fSensitiveVolumeEnergy << " keV"
       << endl;
  cout << "Source origin : (" << fPrimaryEventOrigin.X() << ","
       << fPrimaryEventOrigin.Y() << "," << fPrimaryEventOrigin.Z() << ") mm"
       << endl;

  for (int n = 0; n < GetNumberOfPrimaries(); n++) {
    TVector3* dir = &fPrimaryEventDirection[n];
    cout << "Source " << n
         << " Particle name : " << GetPrimaryEventParticleName(n) << endl;
    cout << "Source " << n << " direction : (" << dir->X() << "," << dir->Y()
         << "," << dir->Z() << ")" << endl;
    cout << "Source " << n << " energy : " << fPrimaryEventEnergy[n] << " keV"
         << endl;
  }
