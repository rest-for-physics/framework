...
Int_t TRestG4Track::GetProcessID(TString pcsName) {
  ...
  // else if ( pcsName == "He3Step") id = 24;
    ...
    cout << "WARNING : The process " << pcsName
         << " was not found. It must be added to TRestG4Track::GetProcessID()"
         << endl;
  }
...
TString TRestG4Track::GetProcessName(Int_t id) {
  ...
  // else if ( id == 24 ) return "He3Step";
    ...
    cout << "WARNING : The process ID : " << id << " could not be found"
         << endl;
...
void TRestG4Track::PrintTrack(int maxHits) {
  cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
       ...
       << endl;
  cout.precision(10);
  cout << " SubEvent ID : " << fSubEventId
       << " Global timestamp : " << GetGlobalTime() << " seconds" << endl;
  cout.precision(2);
  cout << " Track ID : " << GetTrackID() << " Parent ID : " << GetParentID();
  cout << " Particle : " << GetParticleName()
       << " Time track length : " << GetTrackTimeLength() << " us" << endl;
  ...
  if (maxHits > 0) {
    ...
    cout << " Printing only the first " << nHits << " hits of the track"
         << endl;
  }
  ...
  for (int i = 0; i < nHits; i++) {
    cout << "Hit " << i
         << " process : " << GetProcessName(hits->GetHitProcess(i))
         << " volume : " << hits->GetHitVolume(i) << " X : " << hits->GetX(i)
         << " Y : " << hits->GetY(i) << " Z : " << hits->GetZ(i) << " mm"
