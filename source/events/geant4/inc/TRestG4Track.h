...
class TRestG4Track : public TObject {
 ...
 public:
  ...
  Double_t GetEnergyInVolume(Int_t volID) {
    return GetHits()->GetEnergyInVolume(volID);
  }
  TVector3 GetMeanPositionInVolume(Int_t volID) {
    return GetHits()->GetMeanPositionInVolume(volID);
  }
  TVector3 GetFirstPositionInVolume(Int_t volID) {
    return GetHits()->GetFirstPositionInVolume(volID);
  }
  TVector3 GetLastPositionInVolume(Int_t volID) {
    return GetHits()->GetLastPositionInVolume(volID);
  }
  ...
  void SetTrackOrigin(Double_t x, Double_t y, Double_t z) {
    fTrackOrigin.SetXYZ(x, y, z);
  }
  ...
  Bool_t isRadiactiveDecayInVolume(Int_t volID) {
    for (int n = 0; n < GetHits()->GetNumberOfHits(); n++)
      if ((GetHits()->GetHitProcess(n) == 11) &&
          (GetHits()->GetHitVolume(n)) == volID)
        return true;
    return false;
  }
  Bool_t isPhotoElectricInVolume(Int_t volID) {
    for (int n = 0; n < GetHits()->GetNumberOfHits(); n++)
      if ((GetHits()->GetHitProcess(n) == 3) &&
          (GetHits()->GetHitVolume(n)) == volID)
        return true;
    return false;
  }
  ...
  Bool_t isNeutronInVolume(Int_t volID) {
    for (int n = 0; n < GetHits()->GetNumberOfHits(); n++)
      if ((GetHits()->GetHitVolume(n) == volID) &&
