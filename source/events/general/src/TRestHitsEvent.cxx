...
void TRestHitsEvent::AddHit(TVector3 pos, Double_t en, Double_t t, Short_t mod,
                            Short_t ch) {
...
TRestHits* TRestHitsEvent::GetXZHits() {
  ...
  for (int i = 0; i < this->GetNumberOfHits(); i++)
    if (IsNaN(this->GetY(i)))
      fXZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i),
                      this->GetEnergy(i), 0, this->GetModule(i),
                      this->GetChannel(i));
...
TRestHits* TRestHitsEvent::GetYZHits() {
  ...
  for (int i = 0; i < this->GetNumberOfHits(); i++)
    if (IsNaN(this->GetX(i)))
      fYZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i),
                      this->GetEnergy(i), 0, this->GetModule(i),
                      this->GetChannel(i));
...
TRestHits* TRestHitsEvent::GetXYZHits() {
  ...
  for (int i = 0; i < this->GetNumberOfHits(); i++)
    if (!IsNaN(this->GetX(i)) && !IsNaN(this->GetY(i)) && !IsNaN(this->GetZ(i)))
      fXYZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i),
                       this->GetEnergy(i), 0, this->GetModule(i),
                       this->GetChannel(i));
...
Bool_t TRestHitsEvent::isHitsEventInsideCylinder(TVector3 x0, TVector3 x1,
                                                 Double_t radius) {
...
Bool_t TRestHitsEvent::isHitsEventInsidePrism(TVector3 x0, TVector3 x1,
...
Bool_t TRestHitsEvent::areHitsFullyContainnedInsideCylinder(TVector3 x0,
                                                            TVector3 x1,
                                                            Double_t radius) {
...
Bool_t TRestHitsEvent::areHitsFullyContainnedInsidePrism(
    TVector3 x0, TVector3 x1, Double_t sizeX, Double_t sizeY, Double_t theta) {
  if (fHits->GetNumberOfHitsInsidePrism(x0, x1, sizeX, sizeY, theta) ==
      GetNumberOfHits())
    return true;
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToCylinderWall(
    TVector3 x0, TVector3 x1, Double_t radius) {
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToCylinderTop(
    TVector3 x0, TVector3 x1, Double_t radius) {
  TVector3 axis = x1 - x0;
  Double_t cylLength = axis.Mag();
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToCylinderBottom(
    TVector3 x0, TVector3 x1, Double_t radius) {
  TVector3 axis = x1 - x0;
  Double_t cylLength = axis.Mag();
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToPrismWall(
    TVector3 x0, TVector3 x1, Double_t sizeX, Double_t sizeY, Double_t theta) {
  Double_t dX = sizeX / 2;
  Double_t dY = sizeY / 2;
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToPrismTop(
    TVector3 x0, TVector3 x1, Double_t sizeX, Double_t sizeY, Double_t theta) {
  TVector3 axis = x1 - x0;
...
Double_t TRestHitsEvent::GetClosestHitInsideDistanceToPrismBottom(
    TVector3 x0, TVector3 x1, Double_t sizeX, Double_t sizeY, Double_t theta) {
  TVector3 axis = x1 - x0;
  Double_t prismLength = axis.Mag();
...
void TRestHitsEvent::DrawHistograms(Int_t& column, Double_t pitch,
                                    TString histOption) {
  if (fXYHisto != NULL) {
    delete fXYHisto;
    fXYHisto = NULL;
  ...
  fXZHisto = new TH2F("XZ", "", nBinsX, fMinX - 10, fMinX + pitch * nBinsX,
                      nBinsZ, fMinZ - 10, fMinZ + (pitch / 3) * nBinsZ);
  ...
  if (nXZ > 0) {
    ...
    fXZHisto->GetYaxis()->SetTitleSize(1.4 *
                                       fXZHisto->GetYaxis()->GetTitleSize());
    fXZHisto->GetXaxis()->SetTitleSize(1.4 *
  ...
  if (nYZ > 0) {
    ...
    fYZHisto->GetYaxis()->SetTitleSize(1.4 *
                                       fYZHisto->GetYaxis()->GetTitleSize());
    fYZHisto->GetXaxis()->SetTitleSize(1.4 *
...
void TRestHitsEvent::PrintEvent(Int_t nHits) {
  ...
  cout << "Mean position : ( " << GetMeanPositionX() << " , "
       << GetMeanPositionY() << " , " << GetMeanPositionZ() << " ) " << endl;
