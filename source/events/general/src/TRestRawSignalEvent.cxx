...
Double_t TRestRawSignalEvent::GetIntegralWithThreshold(
  ...
  for (int i = 0; i < GetNumberOfSignals(); i++)
    sum += fSignal[i].GetIntegralWithThreshold(
        from, to, startBaseline, endBaseline, nSigmas, nPointsOverThreshold,
        minPeakAmplitude);

  return sum;
}

TRestRawSignal* TRestRawSignalEvent::GetMaxSignal(Int_t startBin,
                                                  Int_t endBin) {
...
Double_t TRestRawSignalEvent::GetTripleMaxIntegral(Int_t startBin,
                                                   Int_t endBin) {
  Double_t sum = 0;
...
Int_t TRestRawSignalEvent::GetLowestWidth(Int_t startBin, Int_t endBin,
...
Double_t TRestRawSignalEvent::GetAverageWidth(Int_t startBin, Int_t endBin,
                                              Double_t minPeakAmplitude) {
  Double_t avg = 0;
  Int_t n = 0;
  for (int sgnl = 0; sgnl < GetNumberOfSignals(); sgnl++) {
...
Double_t TRestRawSignalEvent::GetLowAverageWidth(Int_t nSignals, Int_t startBin,
                                                 Int_t endBin,
                                                 Double_t minPeakAmplitude) {
...
Double_t TRestRawSignalEvent::GetBaseLineSigmaAverage(Int_t startBin,
                                                      Int_t endBin) {
  Double_t baseLineSigmaMean = 0;

  for (int sgnl = 0; sgnl < GetNumberOfSignals(); sgnl++) {
    Double_t baselineSigma =
        GetSignal(sgnl)->GetBaseLineSigma(startBin, endBin);
