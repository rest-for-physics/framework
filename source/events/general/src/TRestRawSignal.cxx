...
Double_t TRestRawSignal::GetIntegralWithThreshold(
    ...
    if (GetData(i) > baseline + pointThreshold) {
      ...
      if (nPoints >= nPointsOverThreshold) {
        ...
        if (sig > signalThreshold) {
          for (int j = i - nPoints - fHeadPoints;
               j < i + fTailPoints && j < GetNumberOfPoints(); j++) {
            if (j < 0) continue;
            //				if( debug )
...
Double_t TRestRawSignal::GetRiseSlope() {
  ...
  if (fPointsOverThreshold.size() < 2) {
    cout << "REST Warning. TRestRawSignal::GetRiseSlope. Less than 2 points!."
         << endl;
    return 0;
  }
...
Double_t TRestRawSignal::GetBaseLineSigma(Int_t startBin, Int_t endBin,
                                          Double_t baseline) {
