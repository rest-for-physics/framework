...
Bool_t TRestHits::areXYZ() {
  if (!isNaN(0) && (IsNaN(GetX(0)) || IsNaN(GetY(0)) || IsNaN(GetZ(0))))
    return false;
  return true;
}
...
Bool_t TRestHits::isHitNInsidePrism(Int_t n, TVector3 x0, TVector3 x1,
  ...
  if ((l > 0) && (l < prismLength))
    if ((TMath::Abs(hitPos.X()) < sizeX / 2) &&
        (TMath::Abs(hitPos.Y()) < sizeY / 2))
      return true;

  return false;
}

Double_t TRestHits::GetEnergyInPrism(TVector3 x0, TVector3 x1, Double_t sizeX,
...
Int_t TRestHits::GetNumberOfHitsInsidePrism(TVector3 x0, TVector3 x1,
                                            Double_t sizeX, Double_t sizeY,
                                            Double_t theta) {
  Int_t hits = 0;
...
Bool_t TRestHits::isHitNInsideCylinder(Int_t n, TVector3 x0, TVector3 x1,
                                       Double_t radius) {
...
Double_t TRestHits::GetEnergyInCylinder(Int_t i, Int_t j, Double_t radius) {
  return GetEnergyInCylinder(this->GetPosition(i), this->GetPosition(j),
                             radius);
}

Double_t TRestHits::GetEnergyInCylinder(TVector3 x0, TVector3 x1,
                                        Double_t radius) {
  Double_t energy = 0.;
  for (int n = 0; n < GetNumberOfHits(); n++) {
...
Int_t TRestHits::GetNumberOfHitsInsideCylinder(Int_t i, Int_t j,
                                               Double_t radius) {
  return GetNumberOfHitsInsideCylinder(this->GetPosition(i),
                                       this->GetPosition(j), radius);
}

Int_t TRestHits::GetNumberOfHitsInsideCylinder(TVector3 x0, TVector3 x1,
                                               Double_t radius) {
  Int_t hits = 0;
  for (int n = 0; n < GetNumberOfHits(); n++)
...
Double_t TRestHits::GetEnergyInSphere(Double_t x0, Double_t y0, Double_t z0,
                                      Double_t radius) {
  Double_t sum = 0;
  for (int i = 0; i < GetNumberOfHits(); i++) {
...
Bool_t TRestHits::isHitNInsideSphere(Int_t n, Double_t x0, Double_t y0,
                                     Double_t z0, Double_t radius) {
...
void TRestHits::AddHit(Double_t x, Double_t y, Double_t z, Double_t en,
                       Double_t t, Short_t mod, Short_t ch) {
  fNHits++;
...
void TRestHits::AddHit(TVector3 pos, Double_t en, Double_t t, Short_t mod,
                       Short_t ch) {
  fNHits++;
...
void TRestHits::RotateIn3D(Int_t n, Double_t alpha, Double_t beta,
                           Double_t gamma, TVector3 vMean) {
  TVector3 vHit;
...
void TRestHits::Rotate(Int_t n, Double_t alpha, TVector3 vAxis,
                       TVector3 vMean) {
  TVector3 vHit;
...
Double_t TRestHits::GetSkewXY() {
  ...
  for (int n = 0; n < GetNumberOfHits(); n++) {
    if (!IsNaN(fY[n]))
      skewXY +=
          fEnergy[n] * (meanY - fY[n]) * (meanY - fY[n]) * (meanY - fY[n]);
    if (!IsNaN(fX[n]))
      skewXY +=
          fEnergy[n] * (meanX - fX[n]) * (meanX - fX[n]) * (meanX - fX[n]);
  }
...
Double_t TRestHits::GetSkewZ() {
  ...
  for (int n = 0; n < GetNumberOfHits(); n++) {
    if (!IsNaN(fZ[n]))
      skewZ += fEnergy[n] * (meanZ - fZ[n]) * (meanZ - fZ[n]) * (meanZ - fZ[n]);
  }
  return skewZ /= (totalEnergy * sigmaZ * sigmaZ * sigmaZ);
}

Double_t TRestHits::GetMeanPositionXInPrism(TVector3 x0, TVector3 x1,
                                            Double_t sizeX, Double_t sizeY,
                                            Double_t theta) {
  Double_t meanX = 0;
  Double_t totalEnergy = 0;
...
Double_t TRestHits::GetMeanPositionYInPrism(TVector3 x0, TVector3 x1,
                                            Double_t sizeX, Double_t sizeY,
                                            Double_t theta) {
  Double_t meanY = 0;
  Double_t totalEnergy = 0;
...
Double_t TRestHits::GetMeanPositionZInPrism(TVector3 x0, TVector3 x1,
                                            Double_t sizeX, Double_t sizeY,
                                            Double_t theta) {
  Double_t meanZ = 0;
  Double_t totalEnergy = 0;
...
TVector3 TRestHits::GetMeanPositionInPrism(TVector3 x0, TVector3 x1,
                                           Double_t sizeX, Double_t sizeY,
                                           Double_t theta) {
...
Double_t TRestHits::GetMeanPositionXInCylinder(TVector3 x0, TVector3 x1,
                                               Double_t radius) {
  Double_t meanX = 0;
  Double_t totalEnergy = 0;
  for (int n = 0; n < GetNumberOfHits(); n++) {
...
Double_t TRestHits::GetMeanPositionYInCylinder(TVector3 x0, TVector3 x1,
                                               Double_t radius) {
  Double_t meanY = 0;
  Double_t totalEnergy = 0;
  for (int n = 0; n < GetNumberOfHits(); n++) {
...
Double_t TRestHits::GetMeanPositionZInCylinder(TVector3 x0, TVector3 x1,
                                               Double_t radius) {
  Double_t meanZ = 0;
  Double_t totalEnergy = 0;
  for (int n = 0; n < GetNumberOfHits(); n++) {
...
TVector3 TRestHits::GetMeanPositionInCylinder(TVector3 x0, TVector3 x1,
                                              Double_t radius) {
  TVector3 mean(GetMeanPositionXInCylinder(x0, x1, radius),
                GetMeanPositionYInCylinder(x0, x1, radius),
...
Int_t TRestHits::GetMostEnergeticHitInRange(Int_t n, Int_t m) {
  ...
  if (hit == -1)
    cout << "REST warning : No largest hit found! No hits?" << endl;
  return hit;
}
...
TVector2 TRestHits::GetProjection(Int_t n, Int_t m, TVector3 position) {
  ...
  Double_t transversal =
      TMath::Sqrt(origin.Mag2() - longitudinal * longitudinal);

  return TVector2(longitudinal, transversal);
}

Double_t TRestHits::GetTransversalProjection(TVector3 p0, TVector3 direction,
                                             TVector3 position) {
