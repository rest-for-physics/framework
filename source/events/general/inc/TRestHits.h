...
public:
    ...
    std::vector<Float_t>
        fT;  // [fNHits] Absolute time information for each punctual deposition (units us, 0 is time of decay)
    std::vector<Float_t> fEnergy;  // [fNHits] Energy deposited at each 3-coordinate position (units keV)
    std::vector<Short_t> fMod;     // [fNHits] Module ID on which each punctual deposition was detected (units
                                   // us, 0 is time of decay)
    std::vector<Short_t> fCh;  // [fNHits] Channel ID on which each punctual deposition was detected (units
                               // us, 0 is time of decay)
