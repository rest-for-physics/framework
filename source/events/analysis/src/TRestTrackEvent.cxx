...
void TRestTrackEvent::PrintEvent(Bool_t fullInfo) {
  ...
  cout << "Number of tracks : "
       << GetNumberOfTracks("XYZ") + GetNumberOfTracks("X") +
              GetNumberOfTracks("Y")
       << endl;
...
TPad* TRestTrackEvent::DrawEvent(TString option) {
  ...
  */
  ...
  optList.erase(std::remove(optList.begin(), optList.end(), "print"),
                optList.end());
  optList.erase(std::remove(optList.begin(), optList.end(), "noWarning"),
                optList.end());
  ...
  double maxX = -1e10, minX = 1e10, maxZ = -1e10, minZ = 1e10, maxY = -1e10,
         minY = 1e10;
  ...
  for (int tck = 0; tck < nTracks; tck++) {
    ...
    */
    ...
    for (int nhit = 0; nhit < hits->GetNumberOfHits(); nhit++) {
      if (hits->GetNumberOfHits() > maxTrackHits)
        maxTrackHits = hits->GetNumberOfHits();

      if (hits->isNaN(nhit)) {
        cout << "REST Warning. TRestTrackEvent::Draw. Hit is not defined!!"
             << endl;
        getchar();
        continue;
      }
