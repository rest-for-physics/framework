...
void TRest2DHitsEvent::SetXZSignal(int zIndex, double xPosition,
                                   double energy) {
  double sum = 0;
  for (int i = 0; i < fNz; i++) {
...
void TRest2DHitsEvent::SetYZSignal(int zIndex, double yPosition,
                                   double energy) {
  double sum = 0;
  for (int i = 0; i < fNz; i++) {
...
TVector2 TRest2DHitsEvent::GetZRange() {
  ...
  if (low > up) {
    return TVector2(numeric_limits<double>::quiet_NaN(),
...
TPad* TRest2DHitsEvent::DrawEvent(TString option) {
  ...
  if ((GetZRange().Y() - GetZRange().X()) > 0) {
    ...
    if (xzz.size() > 0) {
      pointxz = new TH2D("hxz", "hxz", 100, gxz->GetXmin(), gxz->GetXmax(), 100,
                         gxz->GetYmin(), gxz->GetYmax());
      for (int i = 0; i < xzz.size(); i++) {
    ...
    gxz->SetTitle((TString) "XZ plot, " + ToString(GetNumberOfXZSignals()) +
                  " Signals");
  ...
  if ((GetZRange().Y() - GetZRange().X()) > 0) {
    ...
    if (yzz.size() > 0) {
      pointyz = new TH2D("hxz", "hxz", 100, gyz->GetXmin(), gyz->GetXmax(), 100,
                         gyz->GetYmin(), gyz->GetYmax());
      for (int i = 0; i < yzz.size(); i++) {
    ...
    gyz->SetTitle((TString) "YZ plot, " + ToString(GetNumberOfYZSignals()) +
                  " Signals");
