...
TPad* TRestLinearTrackEvent::DrawEvent(TString option) {
  ...
  fPad->cd(1)->DrawFrame(GetMinLinearLength() - 10, 0,
                         GetMaxLinearLength() + 10,
                         1.05 * GetMaxLinearDeposit());
  fPad->cd(2)->DrawFrame(0, 0, 1.05 * GetMaxTransversalLength(),
                         1.05 * GetMaxTransversalDeposit());
  fPad->cd(3)->DrawFrame(GetMinLinearLength() - 10, 0,
                         GetMaxLinearLength() + 10,
                         1.05 * GetMaxLinearDeposit());
  fPad->cd(4)->DrawFrame(0, 0, 1.05 * GetMaxTransversalLength(),
                         1.05 * GetMaxTransversalDeposit());
  fPad->Draw();
