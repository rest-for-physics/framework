...
Int_t TRestThread::ValidateChain() {
  for (unsigned int i = 0; i < fProcessChain.size() - 1; i++) {
    ...
    if (outEventType != inEventType) {
      ...
      cout << "The event output for process " << fProcessChain[i]->GetName()
           << " is " << outEventType << endl;
      cout << "The event input for process " << fProcessChain[i + 1]->GetName()
           << " is " << inEventType << endl;
      cout << "No events will be processed. Please correct event process "
              "input/output."
           << endl;
...
bool TRestThread::TestRun() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    debug << "Test run " << i << " : Input Event ---- "
          << fInputEvent->ClassName() << "(" << fInputEvent << ")" << endl;
    for (unsigned int j = 0; j < fProcessChain.size(); j++) {
      ...
      debug << " ....  " << ProcessedEvent->ClassName() << "(" << ProcessedEvent
            << ")" << endl;
    }
    ...
    if (fOutputEvent != NULL) {
      debug << "Output Event ---- " << fOutputEvent->ClassName() << "("
            << fOutputEvent << ")" << endl;
      break;
    } else {
...
void TRestThread::PrepareToProcess(bool testrun) {
  debug << "Entering TRestThread::PrepareToProcess( testrun=" << testrun << " )"
        << endl;
  ...
  if (fProcessChain.size() > 0) {
    ...
    for (unsigned int i = 0; i < fProcessChain.size(); i++) {
      ...
      debug << "InitProcess() process for " << fProcessChain[i]->ClassName()
            << endl;
    ...
    if (fInputEvent == NULL) {
      error << "REST ERROR("
            << "In thread " << fThreadId
            << ")::Input event of the first process is not specified!" << endl;
      GetChar();
      exit(1);
    }
    ...
    if (fHostRunner->GetNextevtFunc(fInputEvent, tempTree) != 0) {
      error << "REST ERROR("
            << "In thread " << fThreadId
            << ")::Failed to get the first input event, process cannot start!"
            << endl;
      GetChar();
      exit(1);
    }
    ...
    fAnalysisTree = new TRestAnalysisTree("AnalysisTree_" + ToString(fThreadId),
                                          "dummyTree");
    fAnalysisTree->CreateEventBranches();
    fEventTree =
        new TTree((TString) "EventTree_" + ToString(fThreadId), "dummyTree");
      ...
      if (fTreeBranchDef[i] == "inputevent") {
        TRestEvent* evt = fInputEvent;
        {
          ...
          if (branchesToAdd.size() == 0)
            branchesToAdd.push_back(
                pair<TString, TRestEvent*>(BranchName, evt));
          else
    ...
    if (fEventTree->GetListOfBranches()->GetLast() <
        0)  // if no event branches are created
    {
      delete fEventTree;
      fEventTree = NULL;
    }
    ...
    for (int i = 0; i < fTreeBranchDef.size(); i++) {
      if (fTreeBranchDef[i] == "outputevent") {
        ...
        if (fEventTree->GetBranch(BranchName) ==
            NULL)  // avoid duplicated branch
...
void TRestThread::AddProcess(TRestEventProcess* process) {
  ...
  if (fout.CompatibilityMode() && process->GetVerboseLevel() >= REST_Debug) {
    warning << "REST WARNING! Cannot use \"debug\" output level for process "
  ...
  if (fVerboseLevel >= REST_Debug) {
    ...
    for (unsigned int j = 0; j < fProcessChain.size(); j++) {
      cout << "------- Starting process " +
                  (string)fProcessChain[j]->GetName() + " -------"
           << endl;
    ...
    for (unsigned int j = 0; j < fProcessChain.size(); j++) {
      cout << fProcessChain[j]->ClassName() << "("
    ...
    GetChar("======= End of Event " +
            ((fOutputEvent == NULL) ? ToString(fInputEvent->GetID())
                                    : ToString(fOutputEvent->GetID())) +
            " =======");
  } else {
    for (unsigned int j = 0; j < fProcessChain.size(); j++) {
      fProcessChain[j]->BeginOfEventProcess(ProcessedEvent);
