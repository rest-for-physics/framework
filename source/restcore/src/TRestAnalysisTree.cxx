...
TRestAnalysisTree::TRestAnalysisTree(TString name, TString title)
    : TTree(name, title) {
  SetName(name);
  SetTitle(title);
...
void TRestAnalysisTree::ConnectObservables(TRestAnalysisTree* from) {
  if (from != NULL && !fConnected) {
    ...
    fObservableValues.insert(fObservableValues.begin(), tmpobsval.begin(),
                             tmpobsval.end());
    fObservableNames.insert(fObservableNames.begin(), tmpobsname.begin(),
                            tmpobsname.end());
    fObservableDescriptions.insert(fObservableDescriptions.begin(),
                                   tmpobsdes.begin(), tmpobsdes.end());
    fObservableTypes.insert(fObservableTypes.begin(), tmptypes.begin(),
                            tmptypes.end());
    fNObservables += from->GetNumberOfObservables();

    fConnected = true;
  } else {
    cout << "REST ERROR: AnalysisTree Observables is already connected!"
         << endl;
  }
}
...
Int_t TRestAnalysisTree::AddObservable(TString objName, TRestMetadata* meta,
                                       TString description) {
  if (fBranchesCreated) {
    return -1;
  }
  ...
  if (GetObservableID(brName) == -1) {
    if (ele != NULL && !ele->IsaPointer() && ele->GetName()[0] != 'f') {
      ...
      cout << "Data member \"" << objName << "\" not found in class: \""
           << meta->ClassName() << "\"" << endl;
      return -1;
    }
  } else {
...
void TRestAnalysisTree::PrintObservables(TRestEventProcess* proc, int NObs) {
  if (!isConnected() || !fBranchesCreated) {
    if (fNObservables > 0 &&
        fObservableValues.size() ==
            0)  // the object may be just retrieved from root file
    {
  ...
  if (proc == NULL) {
    ...
    std::cout << "---- AnalysisTree Observable for process: " << proc->GetName()
              << " ----" << std::endl;
  }
  ...
  for (int n = 0; n < GetNumberOfObservables() && n < NObs; n++) {
    if (proc == NULL ||
        (proc != NULL &&
         ((string)fObservableNames[n]).find(proc->GetName()) == 0))
      PrintObservable(n);
  }
...
void TRestAnalysisTree::PrintObservable(int n) {
  ...
  if (isConnected() || fBranchesCreated) {
    if (GetObservableType(n) == "double") {
      std::cout << "Observable : " << ToString(fObservableNames[n], 25)
                << "    Value : " << *(double*)fObservableValues[n]
                << std::endl;
    } else {
      std::cout << "Observable : " << ToString(fObservableNames[n], 25)
                << "    (" << fObservableTypes[n] << ")" << std::endl;
    }
  } else {
    std::cout << "Observable : " << ToString(fObservableNames[n], 25)
              << "    Value : ???" << std::endl;
