...
string TRestMetadata::GetDataMemberValInString(TStreamerElement*aElement)
{
	...
	switch (atype) {
		// basic types
	case TStreamerInfo::kBool: {Bool_t    *val = (Bool_t*)ladd; sprintf(buffer,"%d", *val);  break; }
	case TStreamerInfo::kChar: {Char_t    *val = (Char_t*)ladd; sprintf(buffer,"%d", *val);  break; }
	case TStreamerInfo::kShort: {Short_t   *val = (Short_t*)ladd; sprintf(buffer,"%d", *val);  break; }
	case TStreamerInfo::kInt: {Int_t     *val = (Int_t*)ladd; sprintf(buffer,"%d", *val);  break; }
	case TStreamerInfo::kLong: {Long_t    *val = (Long_t*)ladd; sprintf(buffer,"%ld", *val);  break; }
	case TStreamerInfo::kLong64: {Long64_t  *val = (Long64_t*)ladd; sprintf(buffer,"%lld", *val);  break; }
	case TStreamerInfo::kFloat: {Float_t   *val = (Float_t*)ladd; sprintf(buffer,"%f", *val);  break; }
	case TStreamerInfo::kFloat16: {Float_t   *val = (Float_t*)ladd; sprintf(buffer,"%f", *val);  break; }
	case TStreamerInfo::kDouble: {Double_t  *val = (Double_t*)ladd; sprintf(buffer,"%g", *val);  break; }
	case TStreamerInfo::kDouble32: {Double_t  *val = (Double_t*)ladd; sprintf(buffer,"%g", *val);  break; }
	case TStreamerInfo::kUChar: {UChar_t   *val = (UChar_t*)ladd; sprintf(buffer,"%u", *val);  break; }
	case TStreamerInfo::kUShort: {UShort_t  *val = (UShort_t*)ladd; sprintf(buffer,"%u", *val);  break; }
	case TStreamerInfo::kUInt: {UInt_t    *val = (UInt_t*)ladd; sprintf(buffer,"%u", *val);  break; }
	case TStreamerInfo::kULong: {ULong_t   *val = (ULong_t*)ladd; sprintf(buffer,"%lu", *val);  break; }
	case TStreamerInfo::kULong64: {ULong64_t *val = (ULong64_t*)ladd; sprintf(buffer,"%llu", *val);  break; }
	case TStreamerInfo::kBits: {UInt_t    *val = (UInt_t*)ladd; sprintf(buffer,"%d", *val);  break; }

	//			// array of basic types  array[8]
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kBool: {Bool_t    *val = (Bool_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%c ", val[j]); PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kChar: {Char_t    *val = (Char_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%c ", val[j]); PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kShort: {Short_t   *val = (Short_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%d ", val[j]); PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kInt: {Int_t     *val = (Int_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%d ", val[j]); PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kLong: {Long_t    *val = (Long_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%ld ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kLong64: {Long64_t  *val = (Long64_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%lld ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kFloat: {Float_t   *val = (Float_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%f ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kFloat16: {Float_t   *val = (Float_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%f ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kDouble: {Double_t  *val = (Double_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%g ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kDouble32: {Double_t  *val = (Double_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%g ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kUChar: {UChar_t   *val = (UChar_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%u ", val[j]); PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kUShort: {UShort_t  *val = (UShort_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%u ", val[j]); PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kUInt: {UInt_t    *val = (UInt_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%u ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kULong: {ULong_t   *val = (ULong_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%lu ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kULong64: {ULong64_t *val = (ULong64_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%llu ", val[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetL + TStreamerInfo::kBits: {UInt_t    *val = (UInt_t*)ladd; for (j = 0; j < aleng; j++) { sprintf(buffer,"%d ", val[j]); PrintCR(j, aleng, 5); } break; }

	//					   // pointer to an array of basic types  array[n]
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kBool: {Bool_t   **val = (Bool_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%d ", (*val)[j]);  PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kChar: {Char_t   **val = (Char_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%d ", (*val)[j]);  PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kShort: {Short_t  **val = (Short_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%d ", (*val)[j]);  PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kInt: {Int_t    **val = (Int_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%d ", (*val)[j]);  PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kLong: {Long_t   **val = (Long_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%ld ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kLong64: {Long64_t **val = (Long64_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%lld ", (*val)[j]); PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kFloat: {Float_t  **val = (Float_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%f ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kFloat16: {Float_t  **val = (Float_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%f ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kDouble: {Double_t **val = (Double_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%g ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kDouble32: {Double_t **val = (Double_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%g ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kUChar: {UChar_t  **val = (UChar_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%u ", (*val)[j]);  PrintCR(j, aleng, 20); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kUShort: {UShort_t **val = (UShort_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%u ", (*val)[j]);  PrintCR(j, aleng, 10); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kUInt: {UInt_t   **val = (UInt_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%u ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kULong: {ULong_t  **val = (ULong_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%lu ", (*val)[j]);  PrintCR(j, aleng, 5); } break; }
	//case TStreamerInfo::kOffsetP + TStreamerInfo::kULong64: {ULong64_t**val = (ULong64_t**)ladd; for (j = 0; j < *count; j++) { sprintf(buffer,"%llu ", (*val)[j]); PrintCR(j, aleng, 5); } break; }

							  // array counter //[n]
	case TStreamerInfo::kCounter: {Int_t *val = (Int_t*)ladd;    sprintf(buffer,"%d", *val);  break; }
				   // char *
