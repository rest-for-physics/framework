...
TRestAnalysisPlot::TRestAnalysisPlot(const char* cfgFileName, const char* name)
...
void TRestAnalysisPlot::InitFromConfigFile() {
  ...
  if ((formatDefinition = GetKEYDefinition("labels", position)) != "") {
    ...
    fTicksScaleX =
        StringToDouble(GetFieldValue("ticksScaleX", formatDefinition));
    fTicksScaleY =
        StringToDouble(GetFieldValue("ticksScaleY", formatDefinition));

    fLabelScaleX =
        StringToDouble(GetFieldValue("labelScaleX", formatDefinition));
    fLabelScaleY =
        StringToDouble(GetFieldValue("labelScaleY", formatDefinition));

    fLabelOffsetX =
        StringToDouble(GetFieldValue("labelOffsetX", formatDefinition));
    fLabelOffsetY =
        StringToDouble(GetFieldValue("labelOffsetY", formatDefinition));
  ...
  if ((canvasDefinition = GetKEYDefinition("canvas", position)) != "") {
    ...
    fCanvasDivisions =
        StringTo2DVector(GetFieldValue("divide", canvasDefinition));
  ...
  while ((globalCutString = GetKEYDefinition("globalCut", position)) !=
         "")  // general cuts
  {
    TString cutActive = GetFieldValue("value", globalCutString);

    if (cutActive == "on" || cutActive == "ON" || cutActive == "On" ||
        cutActive == "oN") {
      TString obsName = GetFieldValue("name", globalCutString);
      if (obsName == "Not defined")
        obsName = GetFieldValue("variable", globalCutString);
      else {
        cout << "--W-- REST Warning. <globalCut name=\"var\" is now obsolete."
             << endl;
        cout << "--W-- Please, replace by : <globalCut variable=\"var\" "
             << endl;
        cout << endl;
      }
  ...
  while ((addPlotString = GetKEYStructure("plot", position)) !=
         "NotFound")  // general cuts
  {
    TString plotActive = GetFieldValue("value", addPlotString);

    if (plotActive == "on" || plotActive == "ON" || plotActive == "On" ||
        plotActive == "oN") {
      TString plotName =
          RemoveWhiteSpaces(GetFieldValue("name", addPlotString));
      fPlotNames.push_back(plotName);

      TString saveName =
          RemoveWhiteSpaces(GetFieldValue("save", addPlotString));
      ...
      TString option =
          RemoveWhiteSpaces(GetFieldValue("option", addPlotString));
      ...
      while ((addCutString = GetKEYDefinition("cut", pos, addPlotString)) !=
        ...
        if (cutActive == "on" || cutActive == "ON" || cutActive == "On" ||
          ...
          if (GetVerboseLevel() >= REST_Debug)
            cout << "Adding local cut : " << cutVariable << cutCondition
                 << endl;
  ...
  position = 0;
  while ((addPlotString = GetKEYStructure("histo", position)) !=
         "NotFound")  // general cuts
  {
    TString histoActive = GetFieldValue("value", addPlotString);

    if (histoActive == "on" || histoActive == "ON" || histoActive == "On" ||
        histoActive == "oN") {
      TString histoName =
          RemoveWhiteSpaces(GetFieldValue("name", addPlotString));
...
void TRestAnalysisPlot::AddFile(TString fileName) {
  ...
  if (fClasifyBy == "runTag") {
    ...
    cout << "REST Warning. TRestAnalysisPlot : fClassifyBy not recognized"
         << endl;
...
Int_t TRestAnalysisPlot::GetPlotIndex(TString plotName) {
  ...
  cout << "REST WARNING : GetPlotIndex. Plot name " << plotName << " not found"
       << endl;
  return -1;
}
...
void TRestAnalysisPlot::PlotCombinedCanvas() {
  ...
  for (unsigned int i = 0; i < fLegendName.size(); i++)
    for (unsigned int n = 0; n < fFileNames[i].size(); n++) {
      ...
      r->GetEntry(1);
      if (fStartTime == 0 || anT->GetTimeStamp() < fStartTime)
        fStartTime = anT->GetTimeStamp();

      r->GetEntry(r->GetEntries() - 1);
      if (fEndTime == 0 || anT->GetTimeStamp() > fEndTime)
        fEndTime = anT->GetTimeStamp();
    }
  /* }}} */
  ...
  fCombinedCanvas = new TCanvas("combined", "combined", 0, 0, fCanvasSize.X(),
                                fCanvasSize.Y());

  fCombinedCanvas->Divide((Int_t)fCanvasDivisions.X(),
                          (Int_t)fCanvasDivisions.Y());
  /* }}} */
  ...
  for (unsigned int n = 0; n < fPlotString.size(); n++) {
    ...
    for (unsigned int i = 0; i < fLegendName.size(); i++) {
      ...
      size_t varStart =
          FindNthStringPosition((string)fPlotString[n], 0, ">>", 0);
      size_t varEnd =
          FindNthStringPosition((string)fPlotString[n], varStart, "(", 0);

      TString varName = fPlotString[n](varStart, varEnd - varStart);

      hName.push_back(varName + "_" + fLegendName[i]);
      plotString =
          Replace((string)plotString, (string)varName, (string)hName[i], 0, 1);
      hName[i].Remove(0, 2);
      ...
      for (unsigned int m = 0; m < fFileNames[i].size(); m++) {
        ...
        if (m == 1) {
          ...
          plotString = plotString(
              0, FindNthStringPosition((string)plotString, start, "(", 0));
        }
        ...
        if (trees[i][m]->Draw(plotString, fCutString[n], fPlotOption[n]) ==
            -1) {
          cout << endl;
    ...
    for (unsigned int i = 0; i < fLegendName.size(); i++) {
      Double_t value =
          histCollection[i]->GetBinContent(histCollection[i]->GetMaximumBin());
      if (i == 0)
        maxValue = value;
      ...
      htemp->GetXaxis()->SetLabelSize(fTicksScaleX *
                                      htemp->GetXaxis()->GetLabelSize());
      htemp->GetYaxis()->SetLabelSize(fTicksScaleY *
                                      htemp->GetYaxis()->GetLabelSize());

      htemp->GetXaxis()->SetTitleSize(fLabelScaleX *
                                      htemp->GetXaxis()->GetTitleSize());
      htemp->GetYaxis()->SetTitleSize(fLabelScaleY *
                                      htemp->GetYaxis()->GetTitleSize());

      htemp->GetXaxis()->SetTitleOffset(fLabelOffsetX *
                                        htemp->GetXaxis()->GetTitleOffset());
      htemp->GetYaxis()->SetTitleOffset(fLabelOffsetY *
                                        htemp->GetYaxis()->GetTitleOffset());
  ...
  for (unsigned int n = 0; n < fHistoNames.size(); n++) {
    ...
    if (!h) {
      cout << "REST ERROR. TRestAnalysisPlot. A histogram with name : "
           << fHistoNames[n] << " does not exist in input file" << endl;
      exit(1);
    }
    ...
    TH1D* hNew =
        new TH1D("New_" + (TString)fHistoNames[n], fHistoNames[n], nB, bX, bY);
...
void TRestAnalysisPlot::SavePlotToPDF(TString plotName, TString fileName) {
  ...
  if (index >= 0)
    ...
    cout << "Save to plot failed. Plot name " << plotName << " not found"
         << endl;
}
...
void TRestAnalysisPlot::SavePlotToPDF(Int_t n, TString fileName) {
  ...
  anTree->Draw(fPlotString[n], fCutString[n], fPlotOption[n],
               anTree->GetEntries(), 0);
...
TString TRestAnalysisPlot::ReplaceFilenameTags(TString filename,
                                               TRestRun* run) {
