...
void TRestProcessRunner::BeginOfInit() {
  ...
  if (lastEntry - firstEntry > 0 && eventsToProcess == 0) {
    ...
    eventsToProcess =
        eventsToProcess > 0 ? eventsToProcess : REST_MAXIMUM_EVENTS;
    firstEntry = firstEntry > 0 ? firstEntry : 0;
    lastEntry = lastEntry == firstEntry + eventsToProcess ? lastEntry
                                                          : REST_MAXIMUM_EVENTS;
  }
...
Int_t TRestProcessRunner::ReadConfig(string keydeclare, TiXmlElement* e) {
  if (keydeclare == "addProcess") {
    ...
    info << "adding process " << processType << " \"" << processName << "\""
         << endl;
    for (int i = 0; i < fThreadNumber; i++) {
      ...
      if (p != NULL) {
        if (p->isExternal()) {
          ...
          return 0;
        } else if (p->GetVerboseLevel() >= REST_Debug ||
                   p->singleThreadOnly()) {
          fProcStatus = kIgnore;
          info << "multi-threading is disabled due to process \""
               << p->GetName() << "\"" << endl;
          info << "This process is in debug mode or is single thread only"
               << endl;
...
void TRestProcessRunner::EndOfInit() {
  ...
  if (fRunInfo->GetFileProcess() != NULL) {
    ...
    if (fThreads[0]->GetProcessnum() > 0 &&
        fThreads[0]->GetProcess(0)->GetInputEvent() != NULL) {
  ...
  if (fInputEvent == NULL) {
    error << "Cannot determine input event, validating process chain failed!"
          << endl;
    exit(1);
  }
...
void TRestProcessRunner::ReadProcInfo() {
  ...
  ProcessInfo["LastProcess"] =
      (n == 0 ? ProcessInfo["FirstProcess"]
              : fThreads[0]->GetProcess(n - 1)->GetName());
...
void TRestProcessRunner::RunProcess() {
  ...
  fRunInfo->ResetEntry();
  fRunInfo->SetCurrentEntry(firstEntry);
  bool testrun = ToUpper(GetParameter("testRun", "ON")) == "ON" ||
                 ToUpper(GetParameter("testRun", "ON")) == "TRUE";
  for (int i = 0; i < fThreadNumber; i++) {
  ...
  // print metadata
  if (fRunInfo->GetFileProcess() != NULL) {
    essential << this->ClassName() << ": 1 + " << fProcessNumber
              << " processes loaded, " << fThreadNumber << " threads prepared!"
              << endl;
  } else {
    essential << this->ClassName() << ": " << fProcessNumber
              << " processes loaded, " << fThreadNumber << " threads prepared!"
              << endl;
  }
  if (fVerboseLevel >= REST_Essential) {
    if (fRunInfo->GetFileProcess() != NULL)
      fRunInfo->GetFileProcess()->PrintMetadata();
  ...
  while (fProcStatus == kPause || (fRunInfo->GetInputEvent() != NULL &&
                                   eventsToProcess > fProcessedEvents)) {
  ...
  fout << this->ClassName() << ": " << fProcessedEvents << " processed events"
       << endl;

#ifdef TIME_MEASUREMENT
  info << "Total processing time : " << ((Double_t)deltaTime) / 1000. << " ms"
       << endl;
  info << "Average read time from disk (per event) : "
       << ((Double_t)readTime) / fProcessedEvents / 1000. << " ms" << endl;
  info << "Average process time (per event) : "
       << ((Double_t)(deltaTime - readTime - writeTime)) / fProcessedEvents /
              1000.
       << " ms" << endl;
  info << "Average write time to disk (per event) : "
       << ((Double_t)writeTime) / fProcessedEvents / 1000. << " ms" << endl;
  info << "=" << endl;
#endif
...
void TRestProcessRunner::PauseMenu() {
  ...
  while (1) {
    ...
    if (b == 'v') {
      ...
      cursorUp(mainmenuleng + 2);
      cout.setcolor(COLOR_BOLDYELLOW);
      cout << "Invailed option \"" << (char)b << "\" (key value: " << b << ") !"
           << endl;
      cout.setcolor(COLOR_BOLDWHITE);
...
void TRestProcessRunner::ConfigOutputFile() {
  ...
  char tmpString[256];
  if (fRunInfo->GetFileProcess() != NULL) {
    sprintf(tmpString, "Process-%d. %s", 0,
            fRunInfo->GetFileProcess()->GetName());
    fRunInfo->GetFileProcess()->Write();
  }
  for (int i = 0; i < fProcessNumber; i++) {
    sprintf(tmpString, "Process-%d. %s", i + 1,
            fThreads[0]->GetProcess(i)->GetName());
...
void TRestProcessRunner::PrintProcessedEvents(Int_t rateE) {
  if (fProcStatus == kNormal || fProcStatus == kIgnore) {
    ...
    double speedbyte =
        bytes / (double)printInterval * (double)1000000 / ncalculated;
    ...
    double prog = 0;
    if (eventsToProcess == REST_MAXIMUM_EVENTS &&
        fRunInfo->GetFileProcess() != NULL)
    // Nevents is unknown, reading external data file
    {
      prog =
          fRunInfo->GetBytesReaded() / (double)fRunInfo->GetTotalBytes() * 100;
    ...
    if (fRunInfo->GetFileProcess() != NULL) {
      sprintf(buffer, "%d Events (%.1fMB/s), ", fProcessedEvents,
              speedbyte / 1024 / 1024);
    } else {
    ...
    if (fProcStatus == kNormal) {
      sprintf(buffer, "%.1f min ETA, (Pause: \"p\") ",
              (100 - prog_last) / progspeed / 60);
    } else {
      sprintf(buffer, "%.1f min ETA, (Pause Disabled) ",
              (100 - prog_last) / progspeed / 60);
    }
    string s2(buffer);
    ...
    sprintf(buffer, ("%.1f%[" + MakeProgressBar(prog, barlength) + "]").c_str(),
            prog);
    string s3(buffer);
...
TRestAnalysisTree* TRestProcessRunner::GetAnalysisTree() {
