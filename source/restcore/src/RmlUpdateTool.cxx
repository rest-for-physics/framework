...
bool RmlUpdateTool::Is2_1() {
  if (fFileContent.find("<section") != -1 &&
      fFileContent.find("<TRest") == -1) {
    return true;
  }
  return false;
}
...
bool RmlUpdateTool::UpdateSyntax() {
  ...
  int n = system(("python ${REST_PATH}/scripts/restv2_1ToXml.py " + fFileName +
                  " rest_rml_tmp.rml " + name)
                     .c_str());
...
bool RmlUpdateTool::UpdateStructure() {
  ...
  if (doc->LoadFile("rest_rml_tmp.rml")) {
    ...
    if (fElement->FirstChildElement("TRestManager") != NULL) {
      ...
      TiXmlElement* TRestProcessRunnerElement =
          fElement->FirstChildElement("TRestManager");
      TRestProcessRunnerElement->SetValue("TRestProcessRunner");
      TiXmlElement* TRestRunElement_old =
          fElement->FirstChildElement("TRestRun");
      fElement->InsertBeforeChild(TRestProcessRunnerElement,
                                  *TRestRunElement_old);
      ...
      TString processFile =
          GetParameter("processesFile", TRestProcessRunnerElement, "");
      TRestProcessRunnerElement->RemoveChild(GetElementWithName(
          "parameter", "processesFile", TRestProcessRunnerElement));
      TiXmlElement* addProcessElement =
          TRestProcessRunnerElement->FirstChildElement("addProcess");
      ...
      TiXmlElement* addMetadataElement =
          TRestProcessRunnerElement->FirstChildElement("addMetadata");
      while (addMetadataElement != NULL) {
        if (addMetadataElement->Attribute("type") != NULL &&
          ...
          TString readoutFile =
              GetParameter("readoutFile", TRestProcessRunnerElement, "");
          if (readoutFile != "") {
            addMetadataElement->SetAttribute("file", readoutFile);
            TRestProcessRunnerElement->RemoveChild(GetElementWithName(
                "parameter", "readoutFile", TRestProcessRunnerElement));
          }
        }
        if (addMetadataElement->Attribute("type") != NULL &&
          ...
          if (addMetadataElement->Attribute("name") != NULL) {
            addMetadataElement->SetAttribute(
                "name", Replace(addMetadataElement->Attribute("name"), "V/cm",
                                "Vcm", 0));
          }

          TString gasFile =
              GetParameter("gasFile", TRestProcessRunnerElement, "");
          if (gasFile != "") {
      ...
      TRestProcessRunnerElement->InsertBeforeChild(
          TRestProcessRunnerElement->FirstChildElement(), *parele);

      // some parameters should be translated
      TString analysisString =
          GetParameter("pureAnalysisOutput", TRestProcessRunnerElement, "OFF");
      if (analysisString == "ON" || analysisString == "On" ||
          analysisString == "on") {
      ...
      TiXmlElement* inputFileElement = GetElementWithName(
          "parameter", "inputFile", TRestProcessRunnerElement);
      if (inputFileElement != NULL) {
        TRestRunElement->InsertBeforeChild(TRestRunElement->FirstChildElement(),
                                           *inputFileElement);
        TRestProcessRunnerElement->RemoveChild(inputFileElement);
      }

      if (TRestProcessRunnerElement->FirstChildElement("readoutPlane") !=
          NULL) {
    ...
    if (fElement->FirstChildElement("TRestAnalysisPlot") != NULL &&
      ...
      if (name == NULL) {
        name =
            const_cast<char*>(fElement->FirstChildElement("TRestAnalysisPlot")
                                  ->Attribute("name"));
      }
      if (name == NULL) {
        fElement->FirstChildElement("TRestAnalysisPlot")
            ->SetAttribute("name", "myplot");
        name = (char*)"myplot";
      }
