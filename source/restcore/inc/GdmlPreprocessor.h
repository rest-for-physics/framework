 ...
 public:
  ...
  void ReplaceEntity() {
    ...
    while ((pos = filestr.find("<!ENTITY", pos)) != -1) {
      ...
      string entityfile =
          path + RemoveWhiteSpaces(filestr.substr(pos3, pos4 - pos3));
      ...
      if ((pos5 = filestr.find("&" + entityname + ";")) != -1) {
        if (fileExists(entityfile)) {
          ...
          cout << "GDML ERROR! No matching reference file for entity: \""
               << entityname << "\"" << endl;
          cout << "file name: \"" << entityfile << "\"" << endl;
          exit(0);
        }
      } else {
        cout << "GDML: Warning! redundant entity: \"" << entityname << "\""
             << endl;
      }
      pos++;
    }
  ...
  void ReplaceAttributeWithKeyWord(string keyword) {
    ...
    while ((n = filestr.find(keyword, 0)) != -1) {
      ...
      // cout << target << endl;
      string replace =
          ReplaceMathematicalExpressions(ReplaceEnvironmentalVariables(target));
      // cout << replace << endl;
