...
//! A generic analysis REST process to extract valuable information from a *Geant4* simulation produced by *restG4*.
class TRestGeant4AnalysisProcess:public TRestEventProcess {
    private:
        ...
        /// A pointer to the simulation metadata information stored under TRestG4Metadata
        TRestG4Metadata *fG4Metadata; //!

        /// A vector storing the name of observables (xxxVolumeEDep) related to energy deposition in a particular active volume.
        std::vector <std::string> fEnergyInObservables; //!

        /// A vector storing the active volume ids of observables (xxxVolumeEDep) related to energy deposition in a particular active volume.
        std::vector <Int_t> fVolumeID; //!

        /// A vector storing the name of observables (xxxMeanPosX,Y,Z) related to mean hits position in a particular active volume.
	 ...
	 /// A vector storing the name of observables  related to  processes in a particular active volume.
        ...
        /// A vector storing the name of observables (xxxTrackCounter) related to the number of tracks of the given particle name.
        std::vector <std::string> fTrackCounterObservables; //!

        /// A vector storing the name of the particle related to the track counter observable (xxxTrackCounter).
        std::vector <std::string> fParticleTrackCounter; //!

        /// A vector storing the name of observables (xxxTrackEdep) related to the energy deposited by the given particle name.
        std::vector <std::string> fTracksEDepObservables; //!
        
        /// A vector storing the name of the particle related to the track energy observable (xxxTrackEDep).
        std::vector <std::string> fParticleTrackEdep; //!

#endif
        /// A low energy cut applied to input geant4 events. Events below the threshold will be not further processed.
        Double_t fLowEnergyCut;
        
        /// A high energy cut applied to input geant4 events. Events above the threshold will be not further processed.
