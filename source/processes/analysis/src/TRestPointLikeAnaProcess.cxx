...
TRestEvent* TRestPointLikeAnaProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (k = 0; k < fSignalEvent->GetNumberOfSignals(); k++) {
    ...
    lenp = fSignalEvent->GetSignal(k)->GetMaxTime() -
           fSignalEvent->GetSignal(k)->GetMinTime();

    // The activated bins are found.
    for (l = 0; l < lenp - 1; l++) {
      auxcharge =
          fSignalEvent->GetSignal(k)->GetData(l) - fPedLevel - fThreshold;
  ...
  for (k = 0; k < fSignalEvent->GetNumberOfSignals(); k++) {
    ...
    zmean += fSignalEvent->GetSignal(k)->GetMaxValue() *
             fSignalEvent->GetSignal(k)->GetTime(
                 fSignalEvent->GetSignal(k)->GetMaxIndex());
    ...
    for (l = 0; l < lenp - 1; l++) {
      ...
      if ((auxcharge > 0) && (ptime >= 0) && (lenp < 200)) {
        if ((n2dc == 0) && (nchannel >= xl) && (nchannel <= xr)) {
          ...
          xskew += auxcharge * nchannel * nchannel * nchannel * fPitch *
                   fPitch * fPitch;
        ...
        if ((n2dc == 1) && (nchannel >= yl) && (nchannel <= yr)) {
          ...
          yskew += auxcharge * nchannel * nchannel * nchannel * fPitch *
                   fPitch * fPitch;
  ...
  fAnalysisTree->SetObservableValue("z1Bins",
                                    z1bins);  // Number of bins in Z1 direction
  fAnalysisTree->SetObservableValue("z2Bins",
                                    z2bins);  // Number of bins in Z2 direction
  fAnalysisTree->SetObservableValue("zsize", zsize);  // Size in z direction
  fAnalysisTree->SetObservableValue(
      "totalTimeLength", timeDelay);  // Total time length of the event
