...
/*
    ...
    TString fullPathName = (TString) GetMainDataPath() + "/../../gain/" + fGainFilename;
    ...
    fscanf(calfile, "%[^\n]\n", auxchar);
    sscanf(auxchar, "%lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf \t %lf",
	     &peak, &peakerror, &height, &heighterror, &fwhm, 
	     &fwhmerror, &base, &baseerror, &res, &reserror, &ratio, 
	     &ratioerror, &chisquare);
...
TRestEvent* TRestPointLikeAnaProcess::ProcessEvent( TRestEvent *evInput )
{
    ...
    z1bin.Set(512); z2bin.Set(512); xbin.Set(fNChannels); ybin.Set(fNChannels);
	...
	if( yStrip == 0 )      { n2dc = 0; nchannel = readoutChannel;}			// It is x active channel
	else if( xStrip == 0 ) { n2dc = 1; nchannel = readoutChannel - fNChannels;}	// It is y active channel
	...
	for(l = 0; l < lenp - 1; l++)
	{
	    ...
	    {
		if(n2dc == 0)
		xbin.fArray[nchannel] = 1;
		if(n2dc == 1)
		ybin.fArray[nchannel] = 1;
	    }
	}
    }
  ...
  //getchar();
   ...
   for(k = 0; k < 512; k++)
   {
	...
	zmean += fSignalEvent->GetSignal(k)->GetMaxValue() * fSignalEvent->GetSignal(k)->GetTime( fSignalEvent->GetSignal(k)->GetMaxIndex() );
	z2 += fSignalEvent->GetSignal(k)->GetMaxValue() * fSignalEvent->GetSignal(k)->GetTime( fSignalEvent->GetSignal(k)->GetMaxIndex() ) 
    ...
    xmean /= xcharge; x2 /= xcharge; z1mean /= xcharge; z12 /= xcharge;
    ymean /= ycharge; y2 /= ycharge; z2mean /= ycharge; z22 /= ycharge;
    cxz /= xcharge;   cyz /= ycharge;
    xskew /= xcharge;  yskew /= ycharge;
    zmean /= amplitude; z2 /= amplitude;
    ...
    fAnalysisTree->SetObservableValue( "eventCharge", totcharge );  		//Event charge in ADC
    fAnalysisTree->SetObservableValue( "eventAmplitude", amplitude );  		//Event amplitude in ADC
    //Observables in X
    fAnalysisTree->SetObservableValue( "xCharge", xcharge);			//Charge in X direction in ADC
    fAnalysisTree->SetObservableValue( "xzBins", xbins);			//Multiplicity in x direction
    fAnalysisTree->SetObservableValue( "xMean", xmean);			//Mean position in x direction
    fAnalysisTree->SetObservableValue( "z1Mean", z1mean);			//Mean position in z1 direction
    fAnalysisTree->SetObservableValue( "xSigma", xsigma);			//Sigma in x direction
    fAnalysisTree->SetObservableValue( "z1Sigma", z1sigma);			//Sigma in z1 direction
    fAnalysisTree->SetObservableValue( "xskew", xskew );		         //Skew in x direction
    //Observables in Y	
    fAnalysisTree->SetObservableValue( "yCharge", ycharge );			//Charge in Y direction in ADC
    fAnalysisTree->SetObservableValue( "yzBins", ybins);			//Multiplicity in y direction
    fAnalysisTree->SetObservableValue( "yMean", ymean);			//Mean position in y direction
    fAnalysisTree->SetObservableValue( "z2Mean", z1mean);			//Mean position in z2 direction
    fAnalysisTree->SetObservableValue( "ySigma", ysigma);			//Sigma in y direction
    fAnalysisTree->SetObservableValue( "z2Sigma", z2sigma);			//Sigma in z2 direction
    fAnalysisTree->SetObservableValue( "yskew", yskew );		         //Skew in y direction
    //General observables
    fAnalysisTree->SetObservableValue( "energy", energy);			//Energy in keV
    fAnalysisTree->SetObservableValue( "z1Bins", z1bins);			//Number of bins in Z1 direction
    fAnalysisTree->SetObservableValue( "z2Bins", z2bins);			//Number of bins in Z2 direction
    fAnalysisTree->SetObservableValue( "zsize", zsize );		         //Size in z direction
    fAnalysisTree->SetObservableValue( "totalTimeLength", timeDelay );		//Total time length of the event
