...
void TRestMuonAnalysisProcess::InitProcess() {
  ...
  muhitmap = new TH2D("muhitmap", "muon hitmap on micromegas", (X2 - X1) / 3,
                      X1, X2, (Y2 - Y1) / 3, Y1, Y2);
  muhitdir = new TH2D("muhitdir", "muon hit direction towards center",
                      (X2 - X1) / 3, X1, X2, (Y2 - Y1) / 3, Y1, Y2);
}
...
TRestEvent* TRestMuonAnalysisProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  this->SetObservableValue("adjustedfirstX",
                           numeric_limits<double>::quiet_NaN());
  this->SetObservableValue("adjustedfirstY",
                           numeric_limits<double>::quiet_NaN());
  ...
  if (fAnaEvent->GetSubEventTag() == "muon") {
    if (fAnaEvent->GetHoughXZ().size() == 0 &&
        fAnaEvent->GetHoughYZ().size() == 0)
      fAnaEvent->DoHough();
    ...
    if ((a <= c && c <= b) || (c <= a && a <= d)) {
      ...
      this->SetObservableValue("rawmutanthe", sqrt(tan(thexz) * tan(thexz) +
                                                   tan(theyz) * tan(theyz)));
      double muphi = 0;
      if (tan(thexz) >= 0) {
      ...
      if (thexz == 0) {
        ...
        double xlen2 =
            eve->GetNumberOfXZSignals() * stripWidth * (xlen1 > 0 ? 1 : -1);
      ...
      if (theyz == 0) {
        ...
        double ylen2 =
            eve->GetNumberOfYZSignals() * stripWidth * (ylen1 > 0 ? 1 : -1);
      ...
      firstx =
          eve->GetFirstX() - tan(thexz) * (eve->GetZRangeInXZ().X() - firstz);
      firsty =
          eve->GetFirstY() - tan(theyz) * (eve->GetZRangeInYZ().X() - firstz);
      ...
      this->SetObservableValue(
          "mutanthe", sqrt(tan(thexz) * tan(thexz) + tan(theyz) * tan(theyz)));
      muphi = 0;
      if (tan(thexz) >= 0) {
        muphi = atan(tan(theyz) / tan(thexz));
      } else if (tan(theyz) > 0) {
      ...
      if (xzflag) {
        ...
        for (int i = shift; i < 512 + shift; i++) {
          mudeposxz->SetBinContent(
              mudeposxz->FindBin(i),
              mudeposxz->GetBinContent(mudeposxz->FindBin(i)) +
                  xzdepos[i - shift]);
        }
        nummudeposxz++;
      }
      if (yzflag) {
        ...
        for (int i = shift; i < 512 + shift; i++) {
          mudeposyz->SetBinContent(
              mudeposyz->FindBin(i),
              mudeposyz->GetBinContent(mudeposyz->FindBin(i)) +
                  yzdepos[i - shift]);
        }
        nummudeposyz++;
      }
      ...
      musmearz.clear();
      hdiffz->Reset();
      if (xzflag && firstx > X1 + 30 && firstx < X2 - 30 && firsty > Y1 + 30 &&
          firsty < Y2 - 30) {
        ...
        for (int i = 0; i < eve->GetNumberOfXZSignals(); i++) {
          ...
          if (DistanceToTrack(x, z, firstx, 150, thexz) < 20 && z > 10 &&
              z < 500) {
            for (int j = 0; j < 10; j++) {
      ...
      if (yzflag && firstx > X1 + 30 && firstx < X2 - 30 && firsty > Y1 + 30 &&
          firsty < Y2 - 30) {
        map<int, pair<double, double>> musmearz0;
        double n = 0;
        double sum = 0;
        for (int i = 0; i < eve->GetNumberOfYZSignals(); i++) {
          ...
          double z = max_element(sgn.begin(), sgn.end()) - sgn.begin();
          if (DistanceToTrack(y, z, firsty, 150, theyz) < 20 && z > 10 &&
              z < 500) {
            for (int j = 0; j < 10; j++) {
              hdiffz->SetBinContent(10 - j, sgn[z - j]);
              hdiffz->SetBinContent(10 + j, sgn[z + j]);
...
double TRestMuonAnalysisProcess::ProjectionToCenter(double x, double y,
                                                    double xzthe,
                                                    double yzthe) {
...
double TRestMuonAnalysisProcess::DistanceToTrack(double x, double z, double x0,
                                                 double z0, double theta) {
  double px = (tan(theta) * (z - z0) + tan(theta) * tan(theta) * x + x0) /
              (tan(theta) * tan(theta) + 1);
  double pz = (tan(theta) * z0 + 1 / tan(theta) * z - x0 + x) /
