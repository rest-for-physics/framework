...
void TRestHitsAnalysisProcess::LoadConfig(std::string cfgFilename,
                                          std::string name) {
...
TRestEvent* TRestHitsAnalysisProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  fAnalysisTree->SetObservableValue(obsName,
                                    (nHitsX - nHitsY) / (nHitsX + nHitsY));
  ...
  if ((nHits == nHitsX) || (nHits == nHitsY))
    ...
    fAnalysisTree->SetObservableValue(
        obsName, TMath::Sqrt(nHitsX * nHitsX + nHitsY * nHitsY));
  ...
  if (fCylinderFiducial) {
    ...
    Int_t nCylVol = fOutputHitsEvent->GetNumberOfHitsInsideCylinder(
        fFid_x0, fFid_x1, fFid_R);

    Double_t enCylVol =
        fOutputHitsEvent->GetEnergyInCylinder(fFid_x0, fFid_x1, fFid_R);
  ...
  if (fPrismFiducial) {
    TVector3 meanPositionInPrism = fOutputHitsEvent->GetMeanPositionInPrism(
        fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);

    Int_t isInsidePrism = 0;
    if (fOutputHitsEvent->isHitsEventInsidePrism(fFid_x0, fFid_x1, fFid_sX,
                                                 fFid_sY, fFid_theta))
      isInsidePrism = 1;

    Int_t nPrismVol = fOutputHitsEvent->GetNumberOfHitsInsidePrism(
        fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);

    Double_t enPrismVol = fOutputHitsEvent->GetEnergyInPrism(
        fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);
  ...
  if (fCylinderFiducial) {
    ...
    Double_t dToCylWall =
        fOutputHitsEvent->GetClosestHitInsideDistanceToCylinderWall(
            fFid_x0, fFid_x1, fFid_R);
    Double_t dToCylTop =
        fOutputHitsEvent->GetClosestHitInsideDistanceToCylinderTop(
            fFid_x0, fFid_x1, fFid_R);
    Double_t dToCylBottom =
        fOutputHitsEvent->GetClosestHitInsideDistanceToCylinderBottom(
            fFid_x0, fFid_x1, fFid_R);
  ...
  if (fPrismFiducial) {
    ...
    Double_t dToPrismWall =
        fOutputHitsEvent->GetClosestHitInsideDistanceToPrismWall(
            fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);
    Double_t dToPrismTop =
        fOutputHitsEvent->GetClosestHitInsideDistanceToPrismTop(
            fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);
    Double_t dToPrismBottom =
        fOutputHitsEvent->GetClosestHitInsideDistanceToPrismBottom(
            fFid_x0, fFid_x1, fFid_sX, fFid_sY, fFid_theta);
  ...
  fAnalysisTree->SetObservableValue(obsName,
                                    (energyX - energyY) / (energyX + energyY));
