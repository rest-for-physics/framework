...
TRestEvent* TRestPointLikeTrackAnaProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (int tck = 0; tck < fTrackEvent->GetNumberOfTracks(); tck++) {
    ...
    Int_t numberHits =
        fTrackEvent->GetTrack(tck)->GetVolumeHits()->GetNumberOfHits();
  ...
  for (int cl = 0;
       cl < fTrackEvent->GetTrack(maxIndex)->GetVolumeHits()->GetNumberOfHits();
       cl++) {
    Double_t clusterSize =
        fTrackEvent->GetTrack(maxIndex)->GetVolumeHits()->GetClusterSize(cl);
  ...
  fAnalysisTree->SetObservableValue(
      "nTracks", nTracks);  // Total number of tracks of the event
  fAnalysisTree->SetObservableValue(
      "nTotalHits", nTotalHits);  // Total number of hits of the event
  fAnalysisTree->SetObservableValue(
      "totalEnergy", totEnergy);  // Total number of hits of the event
  fAnalysisTree->SetObservableValue(
      "clusterEnergyBalance", clusterEnergyBalance);  // cluster energy balance
  fAnalysisTree->SetObservableValue(
      "clusterLengthBalance", clusterLengthBalance);  // cluster length balance
  fAnalysisTree->SetObservableValue(
      "hitsBalance", hitsBalance);  // cluster number of hits balance
  fAnalysisTree->SetObservableValue(
      "size", totSize);  // size of the track with more energy
  fAnalysisTree->SetObservableValue("sizeBalance",
