  ...
  if (fReadout != NULL) {
    {
      ...
      while (iter != fChannelsHistos.end()) {
        ...
        if (mod == NULL) {
          ...
          iter->second = new TH1D(
              (TString) "channelHisto" + ToString(iter->first),
              (TString) "Readout Channel Activity of Module " +
                  ToString(iter->first),
...

    {
      ...
      while (iter != fChannelsHitMaps.end()) {
        ...
        if (mod == NULL) {
          ...
          iter->second = new TH2D(
              (TString) "channelHitmap" + ToString(iter->first),
  ...
  if (fReadout != NULL) {
    ...
    if (firstX_id > -1 && firstY_id > -1) {
      ...
      if (mod1 == mod2 && mod1 > -1) {
        int x = -1, y = -1;
        int n =
            fReadout->GetReadoutModuleWithID(mod1)->GetNumberOfChannels() / 2;
        ...
        debug << "TRestReadoutAnalysisProcess. Adding point to hitmap of "
                 "module : "
              << mod1 << endl;
        debug << "Position on module(X, Y) : (" << x << ", " << y << ")"
              << endl;
        debug << "Absolute position:(X, Y) : (" << firstx << ", " << firsty
              << ")" << endl;
      }
    }
    double integral = 0;
    for (int i = 0; i < fSignalEvent->GetNumberOfSignals(); i++) {
      ...
      if (mod != -1 && fChannelsHistos.count(mod) > 0) {
        ...
        for (int j = 0; j < sgn->GetNumberOfPoints(); j++) {
          ...
          sgn->SetPoint(j, timecharge.X(),
                        timecharge.Y() / fModuldeAmplification[mod]);
        }
    ...
    debug << "TRestReadoutAnalysisProcess. Calibrated event energy : "
          << integral << endl;
  }
  return fSignalEvent;
}
...
void TRestReadoutAnalysisProcess::InitFromConfigFile() {
  ...
  for (int i = 0; i < ampdef.size(); i++) {
    ...
    if (amppair.size() == 2) {
      fModuldeAmplification[StringToInteger(amppair[0])] =
          StringToDouble(amppair[1]);
    }
  }
