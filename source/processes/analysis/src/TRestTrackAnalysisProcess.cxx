...
void TRestTrackAnalysisProcess::LoadConfig(std::string cfgFilename,
                                           std::string name) {
...
void TRestTrackAnalysisProcess::InitProcess() {
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("nTracks_LE_") != string::npos) {
      Double_t energy = StringToDouble(
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistLow_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(9, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistHigh_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(10, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistBalance_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(13, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistWeightedLow_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(17, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistWeightedHigh_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(18, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistHigh_X_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(12, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistBalance_X_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(15, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistWeightedLow_X_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(19, fObservables[i].length()).c_str());
  ...
  for (unsigned int i = 0; i < fObservables.size(); i++)
    if (fObservables[i].find("twistWeightedHigh_X_") != string::npos) {
      Double_t tailPercentage = StringToDouble(
          fObservables[i].substr(20, fObservables[i].length()).c_str());
...
TRestEvent* TRestTrackAnalysisProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (int tck = 0; tck < fInputTrackEvent->GetNumberOfTracks(); tck++) {
    ...
    if ((t->isXYZ()) || (t->isXZ()) || (t->isYZ())) {
      ...
      for (unsigned int n = 0; n < fTrack_En_EnergyObservables.size(); n++)
        if (en > fTrack_En_Threshold[n] - fDeltaEnergy &&
            en < fTrack_En_Threshold[n] + fDeltaEnergy)
          nTracks_En[n]++;
    }
  }
  ...
  if (fEnableTwistParameters) {
    ...
    if (tXYZ) {
      ...
      for (unsigned int n = 0; n < fTwistBalanceObservables.size(); n++) {
        ...
        if (twistStart < twistEnd)
          fTwistBalanceValue[n] =
              (twistEnd - twistStart) / (twistEnd + twistStart);
        else
          fTwistBalanceValue[n] =
              (twistStart - twistEnd) / (twistEnd + twistStart);
      }
    ...
    if (tX) {
      ...
      for (unsigned int n = 0; n < fTwistWeightedHighObservables_X.size();
           n++) {
    ...
    if (tY) {
      ...
      for (unsigned int n = 0; n < fTwistWeightedHighObservables_Y.size();
           n++) {
  ...
  if (fInputTrackEvent->GetSecondMaxEnergyTrack() != NULL)
    maxSecondTrackEnergy =
        fInputTrackEvent->GetSecondMaxEnergyTrack()->GetEnergy();

  Double_t maxSecondTrackEnergy_X = 0;
  if (fInputTrackEvent->GetSecondMaxEnergyTrack("X") != NULL)
    maxSecondTrackEnergy_X =
        fInputTrackEvent->GetSecondMaxEnergyTrack("X")->GetEnergy();

  Double_t maxSecondTrackEnergy_Y = 0;
  if (fInputTrackEvent->GetSecondMaxEnergyTrack("Y") != NULL)
    maxSecondTrackEnergy_Y =
        fInputTrackEvent->GetSecondMaxEnergyTrack("Y")->GetEnergy();
  ...
  if (fPreviousEventTime.size() == 100)
    meanRate =
        100. / (fInputTrackEvent->GetTime() - fPreviousEventTime.front());
