...
void TRestFindTrackBlobsProcess::LoadConfig(std::string cfgFilename,
                                            std::string name) {
...
void TRestFindTrackBlobsProcess::InitProcess() {
  ...
  for (unsigned int i = 0; i < fQ1_Observables.size(); i++) {
    Double_t r1 = atof(
        fQ1_Observables[i].substr(4, fQ1_Observables[i].length() - 4).c_str());
    fQ1_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQ2_Observables.size(); i++) {
    Double_t r2 = atof(
        fQ2_Observables[i].substr(4, fQ2_Observables[i].length() - 4).c_str());
    fQ2_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQ1_X_Observables.size(); i++) {
    Double_t r1 = atof(fQ1_X_Observables[i]
                           .substr(6, fQ1_X_Observables[i].length() - 6)
                           .c_str());
    fQ1_X_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQ2_X_Observables.size(); i++) {
    Double_t r2 = atof(fQ2_X_Observables[i]
                           .substr(6, fQ2_X_Observables[i].length() - 6)
                           .c_str());
    fQ2_X_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQ1_Y_Observables.size(); i++) {
    Double_t r1 = atof(fQ1_Y_Observables[i]
                           .substr(6, fQ1_Y_Observables[i].length() - 6)
                           .c_str());
    fQ1_Y_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQ2_Y_Observables.size(); i++) {
    Double_t r2 = atof(fQ2_Y_Observables[i]
                           .substr(6, fQ2_Y_Observables[i].length() - 6)
                           .c_str());
    fQ2_Y_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQhigh_Observables.size(); i++) {
    Double_t r1 = atof(fQhigh_Observables[i]
                           .substr(7, fQhigh_Observables[i].length() - 7)
                           .c_str());
    fQhigh_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQlow_Observables.size(); i++) {
    Double_t r2 = atof(fQlow_Observables[i]
                           .substr(6, fQlow_Observables[i].length() - 6)
                           .c_str());
    fQlow_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQhigh_X_Observables.size(); i++) {
    Double_t r1 = atof(fQhigh_X_Observables[i]
                           .substr(9, fQhigh_X_Observables[i].length() - 9)
                           .c_str());
    fQhigh_X_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQlow_X_Observables.size(); i++) {
    Double_t r2 = atof(fQlow_X_Observables[i]
                           .substr(8, fQlow_X_Observables[i].length() - 8)
                           .c_str());
    fQlow_X_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQhigh_Y_Observables.size(); i++) {
    Double_t r1 = atof(fQhigh_Y_Observables[i]
                           .substr(9, fQhigh_Y_Observables[i].length() - 9)
                           .c_str());
    fQhigh_Y_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQlow_Y_Observables.size(); i++) {
    Double_t r2 = atof(fQlow_Y_Observables[i]
                           .substr(8, fQlow_Y_Observables[i].length() - 8)
                           .c_str());
    fQlow_Y_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQbalance_Observables.size(); i++) {
    Double_t r1 = atof(fQbalance_Observables[i]
                           .substr(10, fQbalance_Observables[i].length() - 10)
                           .c_str());
    fQbalance_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQratio_Observables.size(); i++) {
    Double_t r2 = atof(fQratio_Observables[i]
                           .substr(8, fQratio_Observables[i].length() - 8)
                           .c_str());
    fQratio_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQbalance_X_Observables.size(); i++) {
    Double_t r1 = atof(fQbalance_X_Observables[i]
                           .substr(12, fQbalance_X_Observables[i].length() - 12)
                           .c_str());
    fQbalance_X_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQratio_X_Observables.size(); i++) {
    Double_t r2 = atof(fQratio_X_Observables[i]
                           .substr(10, fQratio_X_Observables[i].length() - 10)
                           .c_str());
    fQratio_X_Radius.push_back(r2);
  }

  for (unsigned int i = 0; i < fQbalance_Y_Observables.size(); i++) {
    Double_t r1 = atof(fQbalance_Y_Observables[i]
                           .substr(12, fQbalance_Y_Observables[i].length() - 12)
                           .c_str());
    fQbalance_Y_Radius.push_back(r1);
  }

  for (unsigned int i = 0; i < fQratio_Y_Observables.size(); i++) {
    Double_t r2 = atof(fQratio_Y_Observables[i]
                           .substr(10, fQratio_Y_Observables[i].length() - 10)
                           .c_str());
  ...
  for (unsigned int t = 0; t < tracks.size(); t++) {
    ...
    if (tracks[t]->isXYZ()) {
      ...
      for (unsigned int n = 0; n < fQhigh_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1, y1, z1, fQhigh_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2, y2, z2, fQhigh_Radius[n]);

        Double_t q = q2;
        if (q1 > q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQhigh_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQlow_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1, y1, z1, fQlow_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2, y2, z2, fQlow_Radius[n]);

        Double_t q = q2;
        if (q1 < q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQlow_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQbalance_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1, y1, z1, fQbalance_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2, y2, z2, fQbalance_Radius[n]);
        ...
        obsName =
            this->GetName() + (TString) "." + (TString)fQbalance_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, qBalance);
      }

      for (unsigned int n = 0; n < fQratio_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1, y1, z1, fQratio_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2, y2, z2, fQratio_Radius[n]);
        ...
        obsName =
            this->GetName() + (TString) "." + (TString)fQratio_Observables[n];
    ...
    if (tracks[t]->isXZ()) {
      for (unsigned int n = 0; n < fQ1_X_Observables.size(); n++) {
        Double_t q =
            originHits->GetEnergyInSphere(x1_X, 0, z1_X, fQ1_X_Radius[n]);

        obsName =
            this->GetName() + (TString) "." + (TString)fQ1_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQ2_X_Observables.size(); n++) {
        Double_t q =
            originHits->GetEnergyInSphere(x2_X, 0, z2_X, fQ2_X_Radius[n]);

        obsName =
            this->GetName() + (TString) "." + (TString)fQ2_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQhigh_X_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1_X, 0, z1_X, fQhigh_X_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2_X, 0, z2_X, fQhigh_X_Radius[n]);

        Double_t q = q2;
        if (q1 > q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQhigh_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQlow_X_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1_X, 0, z1_X, fQlow_X_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2_X, 0, z2_X, fQlow_X_Radius[n]);

        Double_t q = q2;
        if (q1 < q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQlow_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQbalance_X_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1_X, 0, z1_X, fQbalance_X_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2_X, 0, z2_X, fQbalance_X_Radius[n]);
        ...
        obsName = this->GetName() + (TString) "." +
                  (TString)fQbalance_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, qBalance);
      }

      for (unsigned int n = 0; n < fQratio_X_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(x1_X, 0, z1_X, fQratio_X_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(x2_X, 0, z2_X, fQratio_X_Radius[n]);
        ...
        obsName =
            this->GetName() + (TString) "." + (TString)fQratio_X_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, qRatio);
      }
    }
    ...
    if (tracks[t]->isYZ()) {
      for (unsigned int n = 0; n < fQ1_Y_Observables.size(); n++) {
        Double_t q =
            originHits->GetEnergyInSphere(0, y1_Y, z1_Y, fQ1_Y_Radius[n]);

        obsName =
            this->GetName() + (TString) "." + (TString)fQ1_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQ2_Y_Observables.size(); n++) {
        Double_t q =
            originHits->GetEnergyInSphere(0, y2_Y, z2_Y, fQ2_Y_Radius[n]);

        obsName =
            this->GetName() + (TString) "." + (TString)fQ2_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQhigh_Y_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(0, y1_Y, z1_Y, fQhigh_Y_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(0, y2_Y, z2_Y, fQhigh_Y_Radius[n]);

        Double_t q = q2;
        if (q1 > q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQhigh_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQlow_Y_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(0, y1_Y, z1_Y, fQlow_Y_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(0, y2_Y, z2_Y, fQlow_Y_Radius[n]);

        Double_t q = q2;
        if (q1 < q2) q = q1;

        obsName =
            this->GetName() + (TString) "." + (TString)fQlow_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, q);
      }

      for (unsigned int n = 0; n < fQbalance_Y_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(0, y1_Y, z1_Y, fQbalance_Y_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(0, y2_Y, z2_Y, fQbalance_Y_Radius[n]);
        ...
        obsName = this->GetName() + (TString) "." +
                  (TString)fQbalance_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, qBalance);
      }

      for (unsigned int n = 0; n < fQratio_Y_Observables.size(); n++) {
        Double_t q1 =
            originHits->GetEnergyInSphere(0, y1_Y, z1_Y, fQratio_Y_Radius[n]);
        Double_t q2 =
            originHits->GetEnergyInSphere(0, y2_Y, z2_Y, fQratio_Y_Radius[n]);
        ...
        obsName =
            this->GetName() + (TString) "." + (TString)fQratio_Y_Observables[n];
        fAnalysisTree->SetObservableValue(obsName, qRatio);
      }
    }
  }
...
void TRestFindTrackBlobsProcess::InitFromConfigFile() {
  fHitsToCheckFraction =
      StringToDouble(GetParameter("hitsToCheckFraction", "0.2"));
}
