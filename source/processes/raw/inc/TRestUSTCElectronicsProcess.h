...
struct USTCDataFrame {
	...
	//header | board number | event time | channel id(0~63) | event id | [chip id + data(0~4095)]*512 | ending
	USTCDataFrame()
	{
		boardId = 0;
		chipId = 0;
...
public:
	...
	ClassDef(TRestUSTCElectronicsProcess, 1);      // Template for a REST "event process" class inherited from TRestEventProcess
};
#endif
