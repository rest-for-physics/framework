...
TRestUSTCElectronicsProcess::TRestUSTCElectronicsProcess(char *cfgFileName)
{
	Initialize();
}
	...
	for (int i = 0; i < fEventBuffer[fCurrentBuffer].size(); i++) 
	{
		...
		if (frame->evId == fCurrentEvent&&frame->eventTime==evtTime) 
		{
			...
			fSignalEvent->AddSignal(sgnl);
				...
				debug << "AsAdId, AgetId, chnId, max value: " << frame->boardId<<", "<<frame->chipId << ", " << frame->channelId << ", " << sgnl.GetMaxValue() << endl;
...
bool TRestUSTCElectronicsProcess::ReadFrameData(USTCDataFrame& frame)
{
	...
	//EEEE | E0A0 | 246C 0686 4550 504E | 0001 | 2233 4455 6677 | (A098)(A09C)... | FFFF
	//0~1header | 2~3board number | 4~11event time | 12~13channel id(0~63) | 14~19event id | [chip id + data(0~4095)]*512 | ending
	frame.boardId = frame.data[2] & 0x0F;
	frame.chipId = (frame.data[3] & 0xF0) / 16 - 10;
	frame.readoutType = frame.data[3] & 0x0F;
	Long64_t tmp = (Long64_t)frame.data[5] * 0x10000 + (Long64_t)frame.data[6] * 0x100 + (Long64_t)frame.data[7];//we omit the first byte in case the number is too large
	frame.eventTime = tmp * 0x100000000 + (Long64_t)frame.data[8] * 0x1000000 + (Long64_t)frame.data[9] * 0x10000 + (Long64_t)frame.data[10] * 0x100 + (Long64_t)frame.data[11];
	frame.channelId = frame.data[12] * 0x100 + frame.data[13];
	frame.evId = (frame.data[16] & 0x7F) * 0x1000000 + frame.data[17] * 0x10000 + frame.data[18] * 0x100 + frame.data[19];//we omit the first 17 bits in case the number is too large

	frame.signalId = frame.boardId * 4 * 64 + frame.chipId * 64 + frame.channelId;

#else
	//EEEE | E0A0 | 246C 0686 | 0001 | 2233 | (A098)(A09C)... | FFFF
	//0~1header | 2~3board number | 4~7event time | 8~9channel id(0~63) | 10~11event id | [chip id + data(0~4095)]*512 | ending
	frame.boardId = frame.data[2] & 0x0F;
	frame.chipId = (frame.data[3] & 0xF0) / 16 - 10;
	frame.readoutType = frame.data[3] & 0x0F;
	Long64_t tmp = (Long64_t)frame.data[4] * 0x1000000+ (Long64_t)frame.data[5] * 0x10000 + (Long64_t)frame.data[6] * 0x100 + (Long64_t)frame.data[7];
	frame.eventTime = tmp;
	...
	if (frame.data[N-4]*0x1000000+frame.data[N-3]*0x10000+frame.data[N-2]*0x100+frame.data[N-1] != 0xFFFFFFFF)
	{
		warning << "wrong ending of frame! Event Id : " << frame.evId <<" Channel Id : "<< frame.channelId << endl;
		return false;
	}
...
bool TRestUSTCElectronicsProcess::AddBuffer(USTCDataFrame& frame) 
{
	...
	if (frame.evId >= fCurrentEvent + fEventBuffer.size())
	{
		...
		warning << "this may due to the inconherence of event id. Increase the buffer number!" << endl;
		warning << "Current Event, Burrfering event : " << fCurrentEvent << ", " << frame.evId << endl;
		return false;
	}
	if (frame.evId < fCurrentEvent) {
		warning << "skipping a signal from old event!" << endl;
		warning << "the cause may be that too much events are mixing. Increase the buffer number!" << endl;
		warning << "Current Event, Burrfering event : " << fCurrentEvent << ", " << frame.evId << endl;
		return false;
	}
