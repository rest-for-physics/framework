...
void TRestUSTCElectronicsProcess::InitProcess() {
  ...
  if (fCurrentEvent != 0) {
    warning << "TRestUSTCElectronicsProcess : first event is not with id 0 !"
            << endl;
...
TRestEvent* TRestUSTCElectronicsProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (int i = 0; i < fEventBuffer[fCurrentBuffer].size(); i++) {
    ...
    if (frame->evId == fCurrentEvent && frame->eventTime == evtTime) {
      ...
      warning << "TRestUSTCElectronicsProcess : unmatched signal frame!"
              << endl;
...
bool TRestUSTCElectronicsProcess::GetNextFrame(USTCDataFrame& frame) {
  if (fInputFiles[fCurrentFile] == NULL) {
    if (fCurrentFile <
        fInputFiles.size() - 1)  // try to get frame form next file
    {
      fCurrentFile++;
  ...
  if (fread(frame.data, N, 1, fInputFiles[fCurrentFile]) != 1 ||
...
bool TRestUSTCElectronicsProcess::ReadFrameData(USTCDataFrame& frame) {
  ...
  if (frame.data[N - 4] * 0x1000000 + frame.data[N - 3] * 0x10000 +
          frame.data[N - 2] * 0x100 + frame.data[N - 1] !=
      0xFFFFFFFF) {
    warning << "wrong ending of frame! Event Id : " << frame.evId
            << " Channel Id : " << frame.channelId << endl;
    return false;
  }
