...
bool TRestUSTCElectronicsProcess::GetNextFrame(USTCDataFrame& frame) {
	...
	while (1) {
		...
		if (fread(Protocol, PROTOCOL_SIZE, 1, fInputFiles[fCurrentFile]) != 1 || feof(fInputFiles[fCurrentFile])) {
		...
		if (!(Protocol[0] ^ 0xac) && !(Protocol[1] ^ 0x0f)) {
			...
			if (flag & 0x1) {
				...
				if (fread(fEnding + PROTOCOL_SIZE, ENDING_SIZE - PROTOCOL_SIZE, 1, fInputFiles[fCurrentFile]) != 1 || feof(fInputFiles[fCurrentFile])) {
					fclose(fInputFiles[fCurrentFile]);
					fInputFiles[fCurrentFile] = NULL;
					return OpenNextFile(frame);
				}
				...
				if (fread(fHeader + PROTOCOL_SIZE, HEADER_SIZE - PROTOCOL_SIZE, 1, fInputFiles[fCurrentFile]) != 1 || feof(fInputFiles[fCurrentFile])) {
					fclose(fInputFiles[fCurrentFile]);
					fInputFiles[fCurrentFile] = NULL;
					return OpenNextFile(frame);
				}
				...
				if (fread(frame.data + PROTOCOL_SIZE, DATA_SIZE - PROTOCOL_SIZE, 1, fInputFiles[fCurrentFile]) != 1 || feof(fInputFiles[fCurrentFile])) {
					fclose(fInputFiles[fCurrentFile]);
					fInputFiles[fCurrentFile] = NULL;
					return OpenNextFile(frame);
				}
...
bool TRestUSTCElectronicsProcess::ReadFrameData(USTCDataFrame& frame) {
	...
	// AC0F        | 401C         | 0300                                     | 010A 3140 0000            | 0500 0000               | .... | .... | 8E95 B452 
	// 0~1Protocol | 2~3 010+size | 4~5: 00000011+ETYPE(2)+ST(1)+SOURCEID(5) | 6~11 time stamp(inverted) | 12~15 event id(inverted) | not used    | ending
	...
	// 0~1Protocol | 2~3 not used | 4~5: 11+card(5)+chip(2)+channel(7) | [0011+data(0~4095)]*512 | ending
