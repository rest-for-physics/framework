...
//______________________________________________________________________________
TRestEvent* TRestMultiFEMINOSToSignalProcess::ProcessEvent(
    TRestEvent* evInput) {
  ...
  while (!endOfEvent) {
    ...
    while (!done) {
      ...
      if ((*sh & PFX_0_BIT_CONTENT_MASK) == PFX_START_OF_BUILT_EVENT) {
        ...
        nb_sh =
            fr_sz / 2;  // number of short words is half the event size in bytes
        ...
        nb_sh =
            fr_sz / 2;  // number of short words is half the frame size in bytes
