...
TRestAFTERToSignalProcess::TRestAFTERToSignalProcess(char *cfgFileName)
{
 Initialize();
}
...
//Bucle till it finds the readed bits equals the payload
   while (frameBits<payload) {
	...
	//cout<<ts<<endl;
	   ...
	   if(eventTime>prevTime)deltaTime = eventTime-prevTime;
	   else deltaTime =(MAX_UNSIGNED_INT-prevTime) + eventTime;
	   ...
	   first=false;
	    ...
	    cout<<"Event data packet header: "<<endl;
            ...
            cout<<" RT " << ((ntohs(pHeader.dcc) & 0x0C00)>>10) << " DCCInd "<<((ntohs(pHeader.dcc) & 0x03F0)>>4);
            ...
            cout<<" U " <<((ntohs(pHeader.hdr) & 0x0800)>>10)<<" FECFlags "<<((ntohs(pHeader.hdr) & 0x03F0)>>4);
            cout<<" Index "<<(ntohs(pHeader.hdr) & 0x000F)<<endl;
  
            cout<<"RawFEM "<<ntohs(pHeader.args)<<" M "<<((ntohs(pHeader.args) & 0x8000)>>15);
            cout<<" N "<<((ntohs(pHeader.args) & 0x4000)>>14)<<" Zero "<<((ntohs(pHeader.args) & 0x1000)>>13);
            cout<<" Arg2 "<<GET_RB_ARG2(ntohs(pHeader.args))<<" Arg2 "<<GET_RB_ARG1(ntohs(pHeader.args))<<endl;
		...
		//and to know which ASIC and FEC it belongs to.
	{
      ...
      if(this->GetVerboseLevel()==REST_Debug)printBits(data);
	       ...
	       if(this->GetVerboseLevel()==REST_Debug)cout<<data<<" Time bin "<<timeBin<<endl;
               ...
               if(this->GetVerboseLevel()==REST_Debug)cout<<"Time bin "<<timeBin<<" ADC: "<<data<<endl;
               timeBin++;
               }
      ...
      if(pay){fread(&dat, sizeof(unsigned short),1, fInputBinFile);frameBits+=sizeof(unsigned short);}
      ...
      eventTime = ts | tl;
        ...
        cout<<"Readed "<< sampleCountRead * sizeof(unsigned short) + sizeof(DataPacketHeader) + sizeof(DataPacketEnd) + sampleCountRead%2 * sizeof(unsigned short)  <<" vs HeadSize "<<ntohs(pHeader.size)<<" Diff "<<ntohs(pHeader.size)- (sampleCountRead + sizeof(DataPacketHeader) + sizeof(DataPacketEnd) + sampleCountRead%2)<<endl;
