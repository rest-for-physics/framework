...
TRestCoBoAsAdToSignalProcess::TRestCoBoAsAdToSignalProcess(char *cfgFileName):TRestRawToSignalProcess(cfgFileName)
{
	Initialize();
}
...
Bool_t TRestCoBoAsAdToSignalProcess::OpenInputCoBoAsAdBinFile ( TString fName )
{
	...
	int year,month,day,hour,minute,second,millisecond; //these parameters have to be extracted from the file name. So do not change the origin binary file name.
	if(fName.Length()!=fnOffset+5 || fnOffset<28) 
	{
		cout<<"Input binary file name type unknown!"<<endl;
		return kFALSE;
	}
	if( fName[fnOffset-24]!='-' || fName[fnOffset-21]!='-' || fName[fnOffset-18]!='T' || fName[fnOffset-15]!=':' || fName[fnOffset-12]!=':' || fName[fnOffset-9]!='.' || fName[fnOffset-5]!='_' )    
	{
		cout<<"Input binary file name unknown!"<<endl;
		return kFALSE;
	}

	year= (int)(fName[fnOffset-28]-48)*1000 + (int)(fName[fnOffset-27]-48)*100 + (int)(fName[fnOffset-26]-48)*10 + (int)(fName[fnOffset-25]-48)*1;
	...
	//cout<<"time: "<<year<<" "<<month<<" "<<day<<" "<<hour<<" "<<minute<<" "<<second<<" "<<millisecond<<endl;

	fStartTimeStamp.Set(year,month,day,hour,minute,second,millisecond*1000000,kTRUE,-8*3600);//Offset for Beijing(local) time
