...
Bool_t TRestCoBoAsAdToSignalProcess::OpenInputCoBoAsAdBinFile(TString fName) {
  ...
  fInputFileNames.push_back(fName);
...
TRestEvent* TRestCoBoAsAdToSignalProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  unsigned int frameSize, frameType, revision, headerSize, itemSize, nItems,
      eventIdx, asadIdx, readOffset, status;
  Long64_t eventTime;
  ...
  frameType =
      (unsigned int)frameHeader[5] * 0x100 + (unsigned int)frameHeader[6];
  if (frameHeader[0] == 0x08 &&
      frameType == 1)  // frameType: 1:partial readout  2:full readout
  {
    frameSize = (unsigned int)frameHeader[1] * 0x10000 +
                (unsigned int)frameHeader[2] * 0x100 +
    ...
    headerSize =
        (unsigned int)frameHeader[8] * 0x100 + (unsigned int)frameHeader[9];
    ...
    itemSize =
        (unsigned int)frameHeader[10] * 0x100 + (unsigned int)frameHeader[11];
    ...
    nItems = (unsigned int)frameHeader[12] * 0x1000000 +
             (unsigned int)frameHeader[13] * 0x10000 +
             (unsigned int)frameHeader[14] * 0x100 +
             (unsigned int)frameHeader[15];
    // cout<<"nItems: "<<nItems<<endl;

    eventTime = (Long64_t)frameHeader[16] * 0x10000000000 +
                (Long64_t)frameHeader[17] * 0x100000000 +
                (Long64_t)frameHeader[18] * 0x1000000 +
                (Long64_t)frameHeader[19] * 0x10000 +
    ...
    eventIdx = (unsigned int)frameHeader[22] * 0x1000000 +
               (unsigned int)frameHeader[23] * 0x10000 +
               (unsigned int)frameHeader[24] * 0x100 +
               (unsigned int)frameHeader[25];
    ...
    readOffset =
        (unsigned int)frameHeader[28] * 0x100 + (unsigned int)frameHeader[29];
    ...
    if (frameSize > 256 && nItems * 4 == frameSize - 256) {
      ...
      for (i = 0; i * 512 < nItems; i++) {
        if ((fread(frameDataP, 2048, 1, fInputBinFile)) != 1 ||
            feof(fInputBinFile)) {
          fclose(fInputBinFile);
          fOutputEvent = NULL;
        ...
        for (j = 0; j < 2048; j += 4) {
          ...
          chanIdx = ((unsigned int)(frameDataP[j] & 0x3f) * 2 +
                     (frameDataP[j + 1] >> 7));
          chTmp = agetIdx * 68 + chanIdx;
          buckIdx = ((unsigned int)(frameDataP[j + 1] & 0x7f) * 4 +
                     (frameDataP[j + 2] >> 6));
          sample = ((unsigned int)(frameDataP[j + 2] & 0x0f) * 0x100 +
                    frameDataP[j + 3]);
    ...
    if (frameSize > 256)  // not needed
    {
      ...
      if (fread(frameDataF, 2048, 136, fInputBinFile) != 136 ||
          feof(fInputBinFile)) {
        fclose(fInputBinFile);
        fOutputEvent = NULL;
        cout << "file end 2" << endl;
