...
void TRestElectronDiffusionProcess::InitProcess() {
  ...
  if (fGas == NULL) {
    ...
    if (fLonglDiffCoeff <= 0)
      fLonglDiffCoeff =
          fGas->GetLongitudinalDiffusion(fElectricField);  // (cm)^1/2
...
TRestEvent* TRestElectronDiffusionProcess::ProcessEvent(TRestEvent* evInput) {
      ...
      for (int p = 0; p < fReadout->GetNumberOfReadoutPlanes(); p++) {
        ...
        if (plane->isZInsideDriftVolume(z)) {
          ...
          while (numberOfElectrons > 0) {
            ...
            if (fAttachment)
              isAttached = (fRandom->Uniform(0, 1) >
                            pow(1 - fAttachment, driftDistance / 10.));
            else
              isAttached = 0;
            ...
            if (isAttached == 0) {
              ...
              if (GetVerboseLevel() >= REST_Extreme)
                cout << "Adding hit. x : " << xDiff << " y : " << yDiff
                     << " z : " << zDiff << " en : "
                     << localWValue * REST_Units::keV / REST_Units::eV << " keV"
                     << endl;
              fOutputHitsEvent->AddHit(
                  xDiff, yDiff, zDiff,
  ...
  if (this->GetVerboseLevel() >= REST_Debug) {
    cout << "TRestElectronDiffusionProcess. Input hits energy : "
         << inputHitsEvent->GetEnergy() << endl;
...
void TRestElectronDiffusionProcess::InitFromConfigFile() {
  ...
  fLonglDiffCoeff =
      StringToDouble(GetParameter("longitudinalDiffusionCoefficient", "0"));
  fTransDiffCoeff =
      StringToDouble(GetParameter("transversalDiffusionCoefficient", "0"));
