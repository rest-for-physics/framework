    ...
    if( fMethod == "R-ball" )
    {
        ...
        for( unsigned int t = 0; t < tracks.size(); t++ )
        {
            ...
            for( int n = 0; n < fDivisions+1; n++ )
            {
                ...
                if( n == fDivisions )
                    refNode = nodeHits->GetNumberOfHits() - 2;
                ...
                if( GetVerboseLevel() >= REST_Debug )
                {
                    ...
                    cout << "Distance to reference node : " << nodeHits->GetHitsPathLength( 0, refNode ) << endl;
                    cout << "------" << endl;
                }
                ...
                if( GetVerboseLevel() >= REST_Debug )
                    cout << "Hit : " << n << " Hits inside cylinder : " << originHits->GetNumberOfHitsInsideCylinder( v0, v1, fRadius ) << endl;
                ...
                for( int l = 0; l < originHits->GetNumberOfHits(); l++ )
                {
                    ...
                    {
                        Double_t r = originHits->GetTransversalProjection( v0, direction, originHits->GetPosition( l ) );
            ...
            if( tracks[t]->isXYZ() )
                chargeCoverage = 1 - missingEnergyRatio;

            if( tracks[t]->isXZ() )
                chargeCoverage_X = 1 - missingEnergyRatio;

            if( tracks[t]->isYZ() )
                chargeCoverage_Y = 1 - missingEnergyRatio;
            /* }}} */
            ...
            if( tracks[t]->isXYZ() )
                transDiffusionXYZ = transDiffusion;

            if( tracks[t]->isXZ() )
                transDiffusionX = transDiffusion;

            if( tracks[t]->isYZ() )
                transDiffusionY = transDiffusion;
            /* }}} */
