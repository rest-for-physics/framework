...
TRestEvent* TRestTrackLinearizationProcess::ProcessEvent(TRestEvent* evInput) {
    ...
    for (int tck = 0; tck < fInputTrackEvent->GetNumberOfTracks(); tck++) {
      ...
      for (int oHit = 0; oHit < originHits->GetNumberOfHits(); oHit++) {
        ...
        Double_t longitudinal =
            fLengthResolution * ((Int_t)(projection.X() / fLengthResolution));
        Double_t transversal =
            fTransversalResolution *
            ((Int_t)(projection.Y() / fTransversalResolution));
  ...
  if (fMethod == "R-ball") {
    ...
    for (unsigned int t = 0; t < tracks.size(); t++) {
      ...
      TRestTrack* originTrack =
          fInputTrackEvent->GetOriginTrackById(tracks[t]->GetTrackID());
      ...
      for (int n = 0; n < fDivisions + 1; n++) {
        ...
        if (GetVerboseLevel() >= REST_Debug) {
          ...
          cout << "Distance to reference node : "
               << nodeHits->GetHitsPathLength(0, refNode) << endl;
          cout << "------" << endl;
        }
        ...
        if (GetVerboseLevel() >= REST_Debug) {
          ...
          sphereCenter.Print();
          cout << "Energy inside radius : " << fRadius << " is " << energy
               << endl;
        }

        linTrack.AddChargeToLongitudinalTrack(100. * dist / totalLength,
                                              energy);
        ...
        if (GetVerboseLevel() >= REST_Debug)
          cout << "Hit : " << n << " Hits inside cylinder : "
               << originHits->GetNumberOfHitsInsideCylinder(v0, v1, fRadius)
               << endl;
        ...
        for (int l = 0; l < originHits->GetNumberOfHits(); l++) {
          ...
          if (hitIsOutside[l] &&
              originHits->isHitNInsideSphere(l, sphereCenter, fRadius))
            hitIsOutside[l] = 0;

          if (originHits->isHitNInsideCylinder(l, v0, v1, 1.5 * fRadius)) {
            Double_t r = originHits->GetTransversalProjection(
                v0, direction, originHits->GetPosition(l));

            if (GetVerboseLevel() >= REST_Extreme)
              cout << "Trans proj : " << r << endl;

            linTrack.AddChargeToTransversalTrack(((Int_t)(3. * r)) * 3.,
                                                 energy);
          }
        }
      }
      ...
      for (unsigned int n = 0; n < hitIsOutside.size(); n++)
        energyMissing += hitIsOutside[n];
...
TVector2 TRestTrackLinearizationProcess::FindProjection(TVector3 position,
                                                        TRestHits* nodes) {
  ...
  if (GetVerboseLevel() >= REST_Extreme) {
    ...
    cout << "-------------" << endl;
    cout << "long : " << projection1.X() << " // tr : " << projection1.Y()
         << endl;
    cout << "-------------" << endl;
    cout << "projection 2 " << endl;
    cout << "-------------" << endl;
    cout << "long : " << projection2.X() << " // tr : " << projection2.Y()
         << endl;
    cout << "-------------" << endl;
    GetChar();
  }
...
Double_t TRestTrackLinearizationProcess::GetReferenceNode(TRestHits* nHits,
                                                          Double_t dist,
                                                          Int_t& ref) {
