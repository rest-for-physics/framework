...
void TRestSignalToHitsProcess::LoadConfig(std::string cfgFilename,
                                          std::string name) {
  ...
  if (fElectricField == PARAMETER_NOT_FOUND_DBL) {
    TRestDetectorSetup* detSetup =
        (TRestDetectorSetup*)this->GetDetectorSetup();
    if (detSetup != NULL) {
...
TRestEvent* TRestSignalToHitsProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (int i = 0; i < numberOfSignals; i++) {
    ...
    if (GetVerboseLevel() >= REST_Debug)
      cout << "Searching readout coordinates for signal ID : " << signalID
           << endl;
    ...
    fReadout->GetPlaneModuleChannel(signalID, planeID, readoutModule,
                                    readoutChannel);

    if (readoutChannel == -1) {
      cout << "REST Warning : Readout channel not found for daq ID : "
           << signalID << endl;
      continue;
    }
    /////////////////////////////////////////////////////////////////////////
    ...
    if (fSignalToHitMethod == "onlyMax") {
      ...
      if (GetVerboseLevel() >= REST_Debug)
        cout << "Adding hit. Time : " << time << " x : " << x << " y : " << y
             << " z : " << z << " Energy : " << energy << endl;

      fHitsEvent->AddHit(x, y, z, energy, 0, (Short_t)readoutModule,
                         (Short_t)readoutChannel);
      ...
      for (int j = 0; j < sgnl->GetNumberOfPoints(); j++) {
        ...
        if (GetVerboseLevel() >= REST_Debug)
          cout << "Adding hit. Time : " << sgnl->GetTime(j) << " x : " << x
               << " y : " << y << " z : " << z << endl;
  ...
  if (this->GetVerboseLevel() >= REST_Info) {
    cout << "TRestSignalToHitsProcess. Hits added : "
         << fHitsEvent->GetNumberOfHits() << endl;
    cout << "TRestSignalToHitsProcess. Hits total energy : "
         << fHitsEvent->GetEnergy() << endl;
  }
