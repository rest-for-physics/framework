...
void TRestSignalZeroSuppresionProcess::LoadConfig(std::string cfgFilename,
                                                  std::string name) {
...
void TRestSignalZeroSuppresionProcess::InitProcess() {
  ...
  for (int i = 0; i < fFriendlyProcesses.size(); i++) {
    if ((string)fFriendlyProcesses[i]->ClassName() ==
        "TRestRawSignalAnalysisProcess") {
      fBaseLineCorrection = true;

      // setting parameters to the same as sAna
      fBaseLineRange = StringTo2DVector(
          fFriendlyProcesses[i]->GetParameter("baseLineRange", "(5,55)"));
      fIntegralRange = StringTo2DVector(
          fFriendlyProcesses[i]->GetParameter("integralRange", "(10,500)"));
      fPointThreshold = StringToDouble(
          fFriendlyProcesses[i]->GetParameter("pointThreshold", "2"));
      fNPointsOverThreshold = StringToInteger(
          fFriendlyProcesses[i]->GetParameter("pointsOverThreshold", "5"));
      fSignalThreshold = StringToDouble(
          fFriendlyProcesses[i]->GetParameter("signalThreshold", "5"));
    }
  }
}
...
//______________________________________________________________________________
TRestEvent* TRestSignalZeroSuppresionProcess::ProcessEvent(
    TRestEvent* evInput) {
    ...
    for (int i = fIntegralRange.X(); i < fIntegralRange.Y(); i++) {
      if (s->GetData(i) > baseline + fPointThreshold * baselinerms) {
        ...
        if (pulse.size() >= fNPointsOverThreshold) {
          ...
          double sq_sum = std::inner_product(pulse.begin(), pulse.end(),
                                             pulse.begin(), 0.0);
            ...
            for (int j = pos; j < i; j++) {
              if (fBaseLineCorrection) {
                if (baseline >= 0 && baseline < 1) {
                  sgn.NewPoint(j * fSampling,
                               (Double_t)s->GetData(j) -
                                   baseline);  // convert timeBin(no unit) to
                                               // time(unit: us)
                } else {
  ...
  debug << "TRestSignalZeroSuppresionProcess. Signals added : "
        << fSignalEvent->GetNumberOfSignals() << endl;
  debug << "TRestSignalZeroSuppresionProcess. Signals rejected : "
        << rejectedSignal << endl;
  debug << "TRestSignalZeroSuppresionProcess. Threshold integral : "
        << totalIntegral << endl;
...
void TRestSignalZeroSuppresionProcess::InitFromConfigFile() {
  ...
  fNPointsFlatThreshold =
      StringToInteger(GetParameter("pointsFlatThreshold", "512"));
