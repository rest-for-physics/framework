...
void TRestTrackReconnectionProcess::LoadConfig(std::string cfgFilename,
                                               std::string name) {
...
void TRestTrackReconnectionProcess::BreakTracks(
    TRestVolumeHits* hits, vector<TRestVolumeHits>& hitSets, Double_t nSigma) {
  hitSets.clear();
  if (GetVerboseLevel() >= REST_Debug) {
  ...
  for (int n = 0; n < hits->GetNumberOfHits(); n++) {
    ...
    if (GetVerboseLevel() >= REST_Debug && n > 0) {
      ...
      if (hits->GetDistance(n - 1, n) > fMeanDistance + nSigma * fSigma)
        cout << " BREAKKKK";
      cout << endl;
    }

    if (n > 0 &&
        hits->GetDistance(n - 1, n) > fMeanDistance + nSigma * fSigma) {
...
void TRestTrackReconnectionProcess::ReconnectTracks(
    vector<TRestVolumeHits>& hitSets) {
  if (GetVerboseLevel() >= REST_Debug) {
    cout << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" << endl;
  ...
  for (int i = 0; i < nSubTracks; i++) {
    for (int j = 0; j < nSubTracks; j++) {
      if (i != j) {
        ...
        Double_t d;
        d = TMath::Sqrt((x1_0 - x2_0) * (x1_0 - x2_0) +
                        (y1_0 - y2_0) * (y1_0 - y2_0) +
                        (z1_0 - z2_0) * (z1_0 - z2_0));
        ...
        d = TMath::Sqrt((x1_0 - x2_1) * (x1_0 - x2_1) +
                        (y1_0 - y2_1) * (y1_0 - y2_1) +
                        (z1_0 - z2_1) * (z1_0 - z2_1));
        ...
        d = TMath::Sqrt((x1_1 - x2_0) * (x1_1 - x2_0) +
                        (y1_1 - y2_0) * (y1_1 - y2_0) +
                        (z1_1 - z2_0) * (z1_1 - z2_0));
        ...
        d = TMath::Sqrt((x1_1 - x2_1) * (x1_1 - x2_1) +
                        (y1_1 - y2_1) * (y1_1 - y2_1) +
                        (z1_1 - z2_1) * (z1_1 - z2_1));
...
Int_t TRestTrackReconnectionProcess::GetTrackBranches(TRestHits& h,
                                                      Double_t nSigma) {
  Int_t breaks = 0;
