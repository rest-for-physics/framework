...
TRestEvent* TRestRawSignalTo2DHitsProcess::ProcessEvent(TRestEvent *evInput)
{
	...
	//if (eve!=NULL && eve->GetNumberOfSignals() > 12 && eve->GetZRange().Y() - eve->GetZRange().X() > 200) {
	//	return eve;
	//}
	//else
	//{
...
//{
//	Foption_t fitOption;
//	ROOT::Fit::FitOptionsMake(ROOT::Fit::kHistogram, option.c_str(), fitOption);
//	ROOT::Fit::DataRange range(h1->GetMinimum(), h1->GetMaximum());
...
//	if (int(fitdata->NDim()) == 0) fitConfig.SetNormErrors(true);
//
//
...
//}
//
//int TRestRawSignalTo2DHitsProcess::Fit(TH1 * h1, TF1 *f1, string option)
//{
//	Foption_t fitOption;
//	ROOT::Fit::FitOptionsMake(ROOT::Fit::kHistogram, option.c_str(), fitOption);
//	ROOT::Fit::DataOptions opt;
//	ROOT::Fit::DataRange range(h1->GetBinCenter(0), h1->GetBinCenter(h1->GetNbinsX() + 1));
//
//
//	Int_t special = f1->GetNumber();
//	Bool_t linear = f1->IsLinear();
...
//
//	if (special != 0 && !fitOption.Bound && !linear) {
//		if (special == 100)      ROOT::Fit::InitGaus(data, f1); // gaussian
//		else if (special == 110 || special == 112)   ROOT::Fit::Init2DGaus(data, f1); // 2D gaussians ( xygaus or bigaus)
//		else if (special == 400)      ROOT::Fit::InitGaus(data, f1); // landau (use the same)
//		else if (special == 410)      ROOT::Fit::Init2DGaus(data, f1); // 2D landau (use the same)
//		else if (special == 200)      ROOT::Fit::InitExpo(data, f1); // exponential
//	}
//
//	auto fitFunction = std::shared_ptr<ROOT::Math::IParamMultiFunction>(new ROOT::Math::WrappedMultiTF1(*f1, f1->GetNdim()));
//	ROOT::Fit::Chi2FCN<ROOT::Math::IMultiGenFunction> func(data, *fitFunction);
//	ROOT::Fit::FitConfig config(f1->GetNpar());
//	ROOT::Math::MinimizerOptions minOption;
//	config.CreateParamsSettings(*fitFunction);
//
//	for (int i = 0; i < f1->GetNpar(); ++i) {
//		ROOT::Fit::ParameterSettings & parSettings = config.ParSettings(i);
//
//		// check limits
//		double plow, pup;
//		f1->GetParLimits(i, plow, pup);
...
//
//
//	auto minimizer = std::shared_ptr<TMinuitMinimizer>(new TMinuitMinimizer());
//
...
//
//	minimizer->SetVariables(config.ParamsSettings().begin(), config.ParamsSettings().end());
//
//	bool ret = minimizer->Minimize();
//
//	ROOT::Fit::FitResult fitResult;
//
//	fitResult.FillResult(minimizer, config, fitFunction, ret, data.Size(), true, NULL);
//
//
//
//	f1->SetChisquare(fitResult.Chi2());
...
void TRestRawSignalTo2DHitsProcess::MakeCluster() {
	...
	if (fOutput2DHitsEvent->GetNumberOfXZSignals() > 3) {
		...
		for (int i = a; i <= b; i += (int)(5 * gRandom->Rndm((Long64_t)this) + 10)) //we sample every 15 points
		{
...
TRest2DHitsEvent* TRestRawSignalTo2DHitsProcess::SelectTag() {
	...
	if (fOutput2DHitsEvent->GetSubEventTag() == "general") //if no tags
	{
		...
		if (TMath::IsNaN(zlen) || fOutput2DHitsEvent->GetZRange().X() < 50 || fOutput2DHitsEvent->GetZRange().X() > 300)
	...
	if (fOutput2DHitsEvent->GetSubEventTag() == "general")
	{
		if (fHough_XZ.size() > 100) {
			...
			{
				fitStatus = hxzt->Fit(fxz, "QN", "", hxzt->GetBinCenter(hxzt->GetMaximumBin()) - 0.2, hxzt->GetBinCenter(hxzt->GetMaximumBin()) + 0.2);
		...
		if (fHough_YZ.size() > 100) {
			...
			{
				fitStatus = hyzt->Fit(fyz, "QN", "", hyzt->GetBinCenter(hyzt->GetMaximumBin()) - 0.2, hyzt->GetBinCenter(hyzt->GetMaximumBin()) + 0.2);
			}
	...
	//0: uses all, 1: muon, 2: strong electron, 3: weak electron, 4: firing, 5: abnormal, 9: other
