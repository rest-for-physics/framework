...
TRestGarfieldDriftProcess::TRestGarfieldDriftProcess(char* cfgFileName)
    : fRandom(0), fGfSensor(0) {
  Initialize();
...
void TRestGarfieldDriftProcess::LoadDefaultConfig() {
  ...
  cout << "TRestGarfieldDriftProcess metadata not found. Loading default values"
       << endl;
...
void TRestGarfieldDriftProcess::InitProcess() {
  ...
  if (!geovol) {
    cout << "REST ERROR when TRestGeometry read GDML file " << fGDML_Filename
         << endl;
    exit(-1);
  }
  ...
  cout << "TRestGarfieldDriftProcess  GetVerboseLevel : "
       << this->GetVerboseLevel() << endl;
  ...
  for (TIter it = thenodes->begin(); it != thenodes->end(); ++it) {
    ...
    if (fGas->GetGDMLMaterialRef() == itmed->GetName()) {
      ...
      if (GetVerboseLevel() >= REST_Info) {
        cout << "  -> gas volume   SetMedium itmed " << itmed->GetName()
             << "  fGas " << fGas->GetGasMedium()->GetName() << endl;
  ...
  cout << "fGeometry " << fGeometry << " nb node "
       << fGeometry->GetNReadoutElecNodes() << endl;
  for (int ii = 0; ii < fGeometry->GetNReadoutElecNodes(); ii++) {
    ...
    cout << "ii " << ii << " readoutnode " << readoutnode << " visible "
         << readoutnode->IsVisible() << endl;
    ...
    readoutmatrix->Print();
    matrixZpos = (readoutmatrix->IsTranslation()
                      ? 10 * readoutmatrix->GetTranslation()[2]
                      : 0);  // converted to mm
    for (int jj = 0; jj < fReadout->GetNumberOfReadoutPlanes(); jj++) {
      ...
      cout << "    jj " << jj << " matrixZpos " << matrixZpos << " planeZpos "
           << planeZpos << endl;
    ...
    if (rdPlaneFound) {
      double field =
          fDriftPotential /
          (fGeometry->GetDriftElecNode()->GetMatrix()->GetTranslation()[2] -
           planeZpos);
      ...
      cout << "add component field " << field << " planeZpos " << planeZpos
           << " planeZvec " << planeZvec << " drift pos "
           << fGeometry->GetDriftElecNode()->GetMatrix()->GetTranslation()[2]
           << endl;
    }
  }
  ...
  if (fGeometry->GetNbOfGfComponent() > 0) {
    ...
    TGeoShape* readoutshape =
        fGeometry->GetReadoutElecNode(0)->GetVolume()->GetShape();
    ...
    double driftelecZpos =
        fGeometry->GetDriftElecNode()->GetMatrix()->GetTranslation()
            [2];  // hope that all these objects are really there...
    // drift area defined from bouding box of readout shape
    fGfSensor->SetArea(
        xmin / 10., ymin / 10., planeZpos / 10. + fStopDistance, xmax / 10.,
        ymax / 10.,
    ...
    printf("  GfSensor area x- %lf y- %lf z- %lf x+ %lf y+ %lf z+ %lf\n",
           xmin / 10., ymin / 10., planeZpos / 10. + 0.2, xmax / 10.,
           ymax / 10., driftelecZpos);
  ...
  if (!fGfDriftMethod || !fGfSensor) {
    cout << "REST ERROR Garfield not fully initialized in "
            "TRestGarfieldDriftProcess::InitProcess "
         << endl;
    cout << "  fGfDriftMethod " << fGfDriftMethod << " fGfSensor " << fGfSensor
         << " GetNbOfGfComponent " << fGeometry->GetNbOfGfComponent() << endl;
    cout << " matrixZpos " << matrixZpos << " planeZpos " << planeZpos
...
Int_t TRestGarfieldDriftProcess::FindModule(Int_t readoutPlane, Double_t x,
                                            Double_t y) {
  // TODO verify this
...
TRestEvent* TRestGarfieldDriftProcess::ProcessEvent(TRestEvent* evInput) {
  ...
  for (int hit = 0; hit < fInputHitsEvent->GetNumberOfHits(); hit++) {
    ...
    unsigned int nb_electrons = fRandom->Poisson(
        nb_electrons_real);  // we need an int number of electrons to drift...
    ...
    for (unsigned int iel = 0; iel < nb_electrons; iel++) {
      ...
      if (fGfDriftMethod->GetNumberOfElectronEndpoints() == 0) {
        cout << " TRestGarfieldDriftProcess::ProcessEvent:  no electron end "
                "point ???"
             << endl;
        continue;
      }
  ...
  if (GetVerboseLevel() >= REST_Debug) {
    cout << "TRestElectronDiffusionProcess. Hits added : "
         << fOutputHitsEvent->GetNumberOfHits() << endl;
    cout << "TRestElectronDiffusionProcess. Hits total energy : "
         << fOutputHitsEvent->GetEnergy() << endl;
    cout << " fTimedHitsEvent " << fOutputHitsEvent << " class "
         << fOutputHitsEvent->ClassName() << endl;
...
void TRestGarfieldDriftProcess::InitFromConfigFile() {
  ...
  fGDML_Filename =
      GetParameter("geometryPath", "") + GetParameter("gdml_file", "");
}
