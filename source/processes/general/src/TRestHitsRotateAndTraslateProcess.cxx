...
//______________________________________________________________________________
TRestEvent* TRestHitsRotateAndTraslateProcess::ProcessEvent(
    TRestEvent* evInput) {
  ...
  for (int hit = 0; hit < fHitsInputEvent->GetNumberOfHits(); hit++) {
    ...
    fHitsOutputEvent->AddHit(proEvent->GetX(hit), proEvent->GetY(hit),
                             proEvent->GetZ(hit), proEvent->GetEnergy(hit));
  }

  if (fHitsOutputEvent->GetNumberOfHits() == 0) return NULL;

  cout << "Electrons rotated: " << fHitsInputEvent->GetNumberOfHits() << "e-s"
       << endl;
