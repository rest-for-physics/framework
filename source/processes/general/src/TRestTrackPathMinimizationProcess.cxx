...
void TRestTrackPathMinimizationProcess::LoadConfig(std::string cfgFilename,
                                                   std::string name) {
...
//______________________________________________________________________________
TRestEvent* TRestTrackPathMinimizationProcess::ProcessEvent(
    TRestEvent* evInput) {
  ...
  for (int tck = 0; tck < fInputTrackEvent->GetNumberOfTracks(); tck++) {
    ...
    if (GetVerboseLevel() >= REST_Warning && nHits > Nmax) {
      ...
      cout << "This number is higher than the recommended maximum for heldkarp "
              "method."
           << endl;
      cout << "Still, you might get a reasonable track path minimization"
           << endl;
    }
    ...
    */
    ...
    if (GetVerboseLevel() >= REST_Warning && rval != 0) {
      cout
          << "REST WARNING. TRestTrackPathMinimizationProcess. HELDKARP FAILED."
          << endl;
    }
