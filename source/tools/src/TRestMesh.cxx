    ...
    for( int i = nx-1; i <= nx+1; i++ )
        for( int j = ny-1; j <= ny+1; j++ )
            for( int k = nz-1; k <= nz+1; k++ )
            {
                ...
                {
                    index = GetNodeIndex( i, j, k );
                    if( index != NODE_NOT_SET && nodeGroupID[index] != nodeGroup )
                        return index;
                }
            }
...
void TRestMesh::SetOrigin( Double_t oX, Double_t oY, Double_t oZ )
{
    ...
    // TODO instead of removing nodes we might need just to re-translate the existing nodes
    RemoveNodes();
}
...
{
    std::cout << "Mesh. Number of nodes : " << GetNumberOfNodes() << " Number of groups : " << GetNumberOfGroups() << std::endl;
    std::cout << "---------------------------------------------" << std::endl;
    for( int i = 0; i < GetNumberOfNodes(); i++ )
        std::cout << "Group : " << nodeGroupID[i] << " X : " <<  nodeX[i] << " Y : " <<  nodeY[i] << " Z : " <<  nodeZ[i] << std::endl;
    std::cout << "---------------------------------------------" << std::endl;
