...
#define CCpq_set_DELETE(x, s, efield)                                              \
    {                                                                              \
        ...
        if ((x)->efield.ptr1) {                                                    \
            CCpq_set_PTR_REPLACE((x)->efield.ptr1->efield, (x), (x)->efield.ptr2); \
        } else {                                                                   \
            if ((s).left == (x)) {                                                 \
        ...
        if ((x)->efield.ptr2) {                                                    \
            CCpq_set_PTR_REPLACE((x)->efield.ptr2->efield, (x), (x)->efield.ptr1); \
        } else {                                                                   \
            if ((s).right == (x)) {                                                \
...
#define CCpq_set_FOREACH(s, x, efield, xprev, xnext)      \
    for ((xprev) = (CCpq_node*)NULL, (x) = (s).left; (x); \
         (xnext) = CCpq_set_PTR_AWAY((x)->efield, xprev), (xprev) = (x), (x) = (xnext))
...
#define CCpq_set_FOREACH_FROM(x, efield, xprev, xnext) \
    for (; (x); (xnext) = CCpq_set_PTR_AWAY((x)->efield, xprev), (xprev) = (x), (x) = (xnext))
...
#define CCpq_set_FOREACH_DEL(s, x, efield, xprev, xnext)                                             \
    for ((xprev) = (CCpq_node*)NULL, (x) = (s).left,                                                 \
        (xnext) = ((x) ? CCpq_set_PTR_AWAY((x)->efield, (CCpq_node*)NULL) : (CCpq_node*)NULL);       \
         (x); (xprev) = (((xprev) ? (((xprev)->efield.ptr1 == (x)) || ((xprev)->efield.ptr2 == (x))) \
                                  : ((s).left == (x)))                                               \
                             ? (x)                                                                   \
                             : (xprev)),                                                             \
        (x) = (xnext), (xnext) = ((x) ? CCpq_set_PTR_AWAY((x)->efield, xprev) : NULL))
...
#define CCpq_set_FOREACH_ADJ(x, efield, z, itemp) \
    for (z = x->efield.ptr1, itemp = 0; itemp < 2; z = x->efield.ptr2, itemp++)
