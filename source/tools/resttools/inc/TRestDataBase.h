...
class TRestDataBase {
 ...
 public:
  ...
  virtual vector<string> query_files(int runnumber, int subrun = 0) {
    return vector<string>(0);
  }
  virtual string query_file(int runnumber, int subrun = 0, int fileid = 0) {
    return "";
  }
  virtual string query_filepattern(int runnumber, int subrun = 0) { return ""; }
  virtual DataBaseFileInfo query_fileinfo(int runnumber, int subrun = 0,
                                          string filename = "") {
  ...
  virtual vector<pair<int, int> > search_tag(string tag) {
    return vector<pair<int, int> >(0);
  }
  virtual vector<pair<int, int> > search_user(string user) {
    return vector<pair<int, int> >(0);
  }
  virtual vector<pair<int, int> > search_type(string type) {
    return vector<pair<int, int> >(0);
  }
  virtual vector<pair<int, int> > search_description(string description) {
    return vector<pair<int, int> >(0);
  }
  virtual vector<pair<int, int> > search_version(string version) {
    return vector<pair<int, int> >(0);
  }
  virtual vector<pair<int, int> > searchexp(string expresstion) {
    return vector<pair<int, int> >(0);
  }

  virtual pair<int, int> getrunwithfilename(string filename) {
    return pair<int, int>();
  }
  ...
  virtual int new_runfile(string filename) {
    return new_runfile(filename, DataBaseFileInfo(filename));
  }
