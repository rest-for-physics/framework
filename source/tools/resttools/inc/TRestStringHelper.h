...
inline std::string ReplaceMathematicalExpressions(std::string buffer) { return REST_StringHelper::ReplaceMathematicalExpressions(buffer); }
...
inline vector<string> GetSubdirectories(const string &path, int recursion = -1) { return REST_StringHelper::GetSubdirectories(path, recursion); }
...
inline std::string SearchFileInPath(string paths, string filename) { return REST_StringHelper::SearchFileInPath(REST_StringHelper::Spilt(paths, ":"), filename); }
inline Int_t ChecktheFile(std::string cfgFileName) { return REST_StringHelper::ChecktheFile(cfgFileName); }
inline std::vector <string> GetFilesMatchingPattern(string pattern) { return REST_StringHelper::GetFilesMatchingPattern(pattern); }
