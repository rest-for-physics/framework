...
std::string Replace(std::string in, std::string thisString,
                    std::string byThisString, size_t fromPosition = 0,
                    Int_t N = 0);
...
Int_t FindNthStringPosition(const string& in, size_t pos,
                            const string& strToFind, size_t nth);
...
inline Int_t isANumber(std::string in) {
  return REST_StringHelper::isANumber(in);
}
inline Int_t isAExpression(std::string in) {
  return REST_StringHelper::isAExpression(in);
}
...
inline std::string EvaluateExpression(std::string exp) {
  return REST_StringHelper::EvaluateExpression(exp);
}
inline Double_t StringToDouble(std::string in) {
  return REST_StringHelper::StringToDouble(in);
}
inline Int_t StringToInteger(std::string in) {
  return REST_StringHelper::StringToInteger(in);
}
inline Bool_t StringToBool(std::string in) {
  return REST_StringHelper::StringToBool(in);
}
inline Long64_t StringToLong(std::string in) {
  return REST_StringHelper::StringToLong(in);
}
inline TVector3 StringTo3DVector(std::string in) {
  return REST_StringHelper::StringTo3DVector(in);
}
inline TVector2 StringTo2DVector(std::string in) {
  return REST_StringHelper::StringTo2DVector(in);
}
inline std::string RemoveWhiteSpaces(std::string in) {
  return REST_StringHelper::RemoveWhiteSpaces(in);
}
inline std::string Replace(std::string in, std::string thisString,
                           std::string byThisString, size_t fromPosition = 0,
                           Int_t N = 0) {
  return REST_StringHelper::Replace(in, thisString, byThisString, fromPosition,
                                    N);
}
inline std::string EscapeSpecialLetters(string in) {
  return REST_StringHelper::EscapeSpecialLetters(in);
}
inline vector<string> Spilt(std::string in, string separator) {
  return REST_StringHelper::Spilt(in, separator);
}
inline Int_t Count(std::string s, std::string sbstring) {
  return REST_StringHelper::Count(s, sbstring);
}
inline Int_t FindNthStringPosition(const string& in, size_t pos,
                                   const string& strToFind, size_t nth) {
  return REST_StringHelper::FindNthStringPosition(in, pos, strToFind, nth);
}
inline string ToDateTimeString(time_t time) {
  return REST_StringHelper::ToDateTimeString(time);
}
...
inline bool fileExists(const std::string& filename) {
  return REST_StringHelper::fileExists(filename);
}
inline bool isRootFile(const std::string& filename) {
  return REST_StringHelper::isRootFile(filename);
}
inline bool isPathWritable(const std::string& path) {
  return REST_StringHelper::isPathWritable(path);
}
inline bool isAbsolutePath(const std::string& path) {
  return REST_StringHelper::isAbsolutePath(path);
}
inline vector<string> GetSubdirectories(const string& path,
                                        int recursion = -1) {
  return REST_StringHelper::GetSubdirectories(path, recursion);
}
inline std::pair<string, string> SeparatePathAndName(
    const std::string fullname) {
  return REST_StringHelper::SeparatePathAndName(fullname);
}
inline string ToAbsoluteName(string filename) {
  return REST_StringHelper::ToAbsoluteName(filename);
}
...
inline std::string SearchFileInPath(string paths, string filename) {
  return REST_StringHelper::SearchFileInPath(
      REST_StringHelper::Spilt(paths, ":"), filename);
}
inline Int_t ChecktheFile(std::string cfgFileName) {
  return REST_StringHelper::ChecktheFile(cfgFileName);
}
...
inline std::vector<TString> GetFilesMatchingPattern(TString pattern) {
  return VectorTString_cast(
      REST_StringHelper::GetFilesMatchingPattern((string)pattern));
}
inline std::string ToUpper(std::string in) {
  return REST_StringHelper::ToUpper(in);
}
inline std::string ExecuteShellCommand(string cmd) {
  return REST_StringHelper::ExecuteShellCommand(cmd);
}
inline int ConvertVersionCode(string in) {
  return REST_StringHelper::ConvertVersionCode(in);
}
