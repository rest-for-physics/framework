...
Int_t REST_StringHelper::isAExpression(string in) {
  string temp = in;
  vector<string> replace{"sqrt", "log", "exp",  "gaus", "cos",
                         "sin",  "tan", "atan", "acos", "asin"};
...
/// input string **buffer**.
///
std::string REST_StringHelper::ReplaceMathematicalExpressions(
    std::string buffer) {
  ...
  if (Expressions.size() > 1 && Expressions[0][0] == '(' &&
      Expressions[Expressions.size() - 1]
                 [Expressions[Expressions.size() - 1].size() - 1] == ')') {
    Expressions[0].erase(0, 1);
    Expressions[Expressions.size() - 1].erase(
        Expressions[Expressions.size() - 1].size() - 1, 1);
    erased = true;
  }
...
Int_t REST_StringHelper::isANumber(string in) {
  return (in.find_first_not_of("-+0123456789.e") == std::string::npos &&
          in.length() != 0);
}
...
///
Int_t REST_StringHelper::FindNthStringPosition(const string& in, size_t pos,
...
/// string **in**.
///
string REST_StringHelper::Replace(string in, string thisString,
                                  string byThisString, size_t fromPosition,
                                  Int_t N) {
  string out = in;
...
time_t REST_StringHelper::ToTime(string time) {
  ...
  time_t time1;
  int i =
      sscanf(time.c_str(), "%d-%d-%d %d:%d:%d", &(tm1.tm_year), &(tm1.tm_mon),
...
std::pair<string, string> REST_StringHelper::SeparatePathAndName(
...
std::string REST_StringHelper::RemoveAbsolutePath(
...
vector<string> REST_StringHelper::GetSubdirectories(const string& path,
                                                    int recursion) {
...
///
std::string REST_StringHelper::SearchFileInPath(vector<string> paths,
                                                string filename) {
