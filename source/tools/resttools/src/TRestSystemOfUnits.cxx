...
bool IsUnit(string unitsStr) {
  if (isEnergy(unitsStr) || isDistance(unitsStr) || isField(unitsStr) ||
      isTime(unitsStr) || isPotential(unitsStr)) {
    return true;
  }
...
Double_t GetValueInRESTUnits(Double_t value, TString unitsStr) {
  ...
  if (isMagneticField(unitsStr))
    return GetMagneticFieldInRESTUnits(value, unitsStr);

  cout << "REST WARNING (REST_Units)  Unit=[" << unitsStr
