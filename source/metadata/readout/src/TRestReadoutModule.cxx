...
void TRestReadoutModule::DoReadoutMapping(Int_t nodes) {
  ...
  for (int ch = 0; ch < this->GetNumberOfChannels(); ch++) {
    for (int px = 0; px < this->GetChannel(ch)->GetNumberOfPixels(); px++) {
      ...
      if (showWarnings && fMapping.isNodeSet(nodeX, nodeY)) {
        ...
        cout << "Trying to associate channel : " << ch << " Pixel : " << px
             << endl;
        cout << "Pixel coordinates : ( " << xPix << " , " << yPix << " ) "
             << endl;

        Int_t tempCh = fMapping.GetChannelByNode(nodeX, nodeY);
        Int_t tempPix = fMapping.GetPixelByNode(nodeX, nodeY);
        cout << "Already associated channel : " << tempCh
             << " pixel : " << tempPix << endl;
        Double_t xP =
            this->GetChannel(tempCh)->GetPixel(tempPix)->GetCenter().X();
        Double_t yP =
            this->GetChannel(tempCh)->GetPixel(tempPix)->GetCenter().Y();
  ...
  for (int i = 0; i < nodes; i++) {
    printf("Completed : %.2lf %%\r",
           100. * (i * (Double_t)nodes) / nodes / nodes);
    fflush(stdout);
    for (int j = 0; j < nodes; j++) {
      ...
      if (!fMapping.isNodeSet(i, j)) {
        for (int ch = 0;
             ch < GetNumberOfChannels() && !fMapping.isNodeSet(i, j); ch++) {
          for (int px = 0; px < GetChannel(ch)->GetNumberOfPixels() &&
                           !fMapping.isNodeSet(i, j);
               px++) {
  ...
  for (int i = 0; i < nodes; i++)
    for (int j = 0; j < nodes; j++) {
      if (!fMapping.isNodeSet(i, j)) {
        ...
        Double_t xAbs = TransformToPhysicalCoordinates(x, y).X();
        Double_t yAbs = TransformToPhysicalCoordinates(x, y).Y();
        cout << "Node NOT SET : " << i << " , " << j << " Mapping x : " << x
             << " y : " << y << endl;
...
Int_t TRestReadoutModule::FindChannel(Double_t absX, Double_t absY) {
  ...
  Int_t totalNodes =
      fMapping.GetNumberOfNodesX() * fMapping.GetNumberOfNodesY();
  ...
  while (!this->isInsidePixel(channel, pixel, absX, absY)) {
    ...
    if (count > totalNodes / 10) {
      cout << "REST Error? I did not find any channel for hit position (" << x
           << "," << y << ")" << endl;
      ...
      for (int ch = 0; ch < GetNumberOfChannels(); ch++)
        for (int px = 0; px < GetChannel(ch)->GetNumberOfPixels(); px++)
          if (isInsidePixel(ch, px, absX, absX)) {
            ...
            cout << "Corresponding node :  nX: "
                 << fMapping.GetNodeX_ForChannelAndPixel(ch, px)
                 << " nY : " << fMapping.GetNodeY_ForChannelAndPixel(ch, px)
                 << endl;
            cout << "Channel : " << ch << " Pixel : " << px << endl;
            cout << "Pix X : " << GetChannel(ch)->GetPixel(px)->GetCenter().X()
                 << " Pix Y : " << GetChannel(ch)->GetPixel(px)->GetCenter().Y()
                 << endl;
          }
      sleep(5);
      //   getchar();
...
///
Bool_t TRestReadoutModule::isInsideChannel(Int_t channel, Double_t x,
                                           Double_t y) {
  TVector2 pos(x, y);
...
///
Bool_t TRestReadoutModule::isInsidePixel(Int_t channel, Int_t pixel, Double_t x,
                                         Double_t y) {
  TVector2 pos(x, y);
...
///
Bool_t TRestReadoutModule::isInsidePixel(Int_t channel, Int_t pixel,
                                         TVector2 pos) {
  if (channel < 0 || pixel < 0) return false;
...
/// returned
///
TVector2 TRestReadoutModule::GetPixelVertex(Int_t channel, Int_t pixel,
                                            Int_t vertex) {
  TVector2 pixPosition =
      GetChannel(channel)->GetPixel(pixel)->GetVertex(vertex);
...
TVector2 TRestReadoutModule::GetPixelVertex(TRestReadoutPixel* pix,
                                            Int_t vertex) {
...
void TRestReadoutModule::AddChannel(TRestReadoutChannel& rChannel) {
  for (int i = 0; i < rChannel.GetNumberOfPixels(); i++) {
    ...
    if (oX + fTolerance < 0 || oY + fTolerance < 0 ||
        sX - fTolerance > fModuleSizeX || sY - fTolerance > fModuleSizeY) {
      if (showWarnings) {
        cout << "REST Warning (AddChannel) pixel outside the module boundaries"
             << endl;
        cout << "Channel: " << fReadoutChannel.size() << ", Pixel : " << i
             << endl;
        cout << "Pixel origin = (" << oX << " , " << oY << ")" << endl;
        cout << "Pixel size = (" << sX << " , " << sY << ")" << endl;
        cout << "Module size = (" << fModuleSizeX << " , " << fModuleSizeY
             << ")" << endl;
      }
    }
  }
