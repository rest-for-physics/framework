...
class TRestReadout : public TRestMetadata {
 private:
  ...
  Bool_t
      fDecoding;  ///< Defines if a decoding file was used to set the relation
  ...
  std::vector<TRestReadoutPlane>
      fReadoutPlanes;  ///< A vector storing the TRestReadoutPlane definitions.

#ifndef __CINT__
  Int_t fMappingNodes;  //!///< Number of nodes per axis used on the readout
                        //!coordinate mapping. See also TRestReadoutMapping.
  vector<TRestReadoutModule>
      fModuleDefinitions;  //!///< A vector storing the different
 ...
 public:
  ...
  void GetPlaneModuleChannel(Int_t daqID, Int_t& planeID, Int_t& moduleID,
                             Int_t& channelID);
  Int_t GetHitsDaqChannel(TVector3 hitpos, Int_t& planeID, Int_t& moduleID,
                          Int_t& channelID);
