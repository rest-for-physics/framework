...
class TRestG4Metadata : public TRestMetadata {
 ...
 public:
  ...
  void SetGeneratorPosition(double x, double y, double z) {
    fGenPosition = TVector3(x, y, z);
  }
  ...
  TRestParticleSource GetSource(int n) {
    return fPrimaryGenerator.GetParticleSource(n);
  }

  /// Returns the name of the particle source with index n (Geant4 based names).
  TRestParticleSource GetParticleSource(int n) {
    return fPrimaryGenerator.GetParticleSource(n);
  }
  ...
  void SetParticleCollection(Int_t n) {
    fPrimaryGenerator.SetSourcesFromParticleCollection(n);
  }
