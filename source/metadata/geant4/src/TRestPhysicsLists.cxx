...
//______________________________________________________________________________
TRestPhysicsLists::TRestPhysicsLists(char* cfgFileName, string name)
...
void TRestPhysicsLists::InitFromConfigFile() {
  ...
  fMinEnergyRangeProductionCuts =
      GetDblParameterWithUnits("minEnergyRangeProductionCuts", 1);
  fMaxEnergyRangeProductionCuts =
      GetDblParameterWithUnits("maxEnergyRangeProductionCuts", 1e6);
  size_t position = 0;
  string physicsListString;

  while ((physicsListString = GetKEYStructure("physicsList", position)) !=
         "NotFound") {
    // PhysicsList name
    TString phName = GetFieldValue(
        "name", GetKEYDefinition("physicsList", physicsListString));

    if (!PhysicsListExists(phName)) {
      cout << "REST WARNING. TRestPhysicsList : Skipping physics list : "
           << phName << endl;
      continue;
    }
    size_t pos = 0;
    ...
    while ((optionDefinition =
                GetKEYDefinition("option", pos, physicsListString)) != "") {
...
TString TRestPhysicsLists::GetPhysicsListOptionValue(TString phName,
                                                     TString option) {
  vector<TString> optList =
      TRestTools::GetOptions(GetPhysicsListOptionString(phName));
...
Bool_t TRestPhysicsLists::PhysicsListExists(TString phName) {
  ...
  cout << "Physics list " << phName << " does NOT exist in TRestPhysicsLists."
       << endl;
  cout << endl;
...
void TRestPhysicsLists::PrintMetadata() {
  ...
  metadata << "Min Energy for particle production: "
           << fMinEnergyRangeProductionCuts << " keV" << endl;
  metadata << "Max Energy for particle production: "
           << fMaxEnergyRangeProductionCuts << " keV" << endl;
  metadata << "---------------------------------------" << endl;
  for (unsigned int n = 0; n < fPhysicsLists.size(); n++) {
