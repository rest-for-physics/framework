...
///
TRestCalibrationCorrection::TRestCalibrationCorrection(const char* configFilename, std::string name) : TRestMetadata(configFilename) {
...
///
void TRestCalibrationCorrection::CalibrateDataSet(const std::string& dataSetFileName, std::string outputFileName){
    ...
    auto calibrate = [this](double val, double x, double y, int pmID) {
        ...
        //RESTError << "TRestCalibrationCorrection::CalibrateDataSet: Module not found for pmID " << pmID << RESTendl;
    ...
    dataFrame = dataFrame.Define(calibObsName, calibrate, 
                {fObservable, fSpatialObservableX,
                 fSpatialObservableY, pmIDname});
...
///
///
TRestCalibrationCorrection::Module* TRestCalibrationCorrection::GetModuleCalibration(const int planeID, const int moduleID){
    for (auto& i : fModulesCal){
...
///
///
double TRestCalibrationCorrection::GetSlopeParameter(const int planeID, const int moduleID, const double x, const double y){
...
///
///
double TRestCalibrationCorrection::GetInterceptParameter(const int planeID, const int moduleID, const double x, const double y){
    Module* moduleCal = GetModuleCalibration(planeID, moduleID);
    if (moduleCal == nullptr) return 0; //return numeric_limits<double>::quiet_NaN()
...
std::set<int> TRestCalibrationCorrection::GetModuleIDs(const int planeId) const {
    ...
    for (const auto& mc : fModulesCal) 
        if (mc.GetPlaneId()==planeId) 
            moduleIDs.insert(mc.GetModuleId());
    return moduleIDs;
...
TRestCalibrationCorrection& TRestCalibrationCorrection::operator=(TRestCalibrationCorrection& src) {
    ...
    for (auto pID : src.GetPlaneIDs())
        for (auto mID : src.GetModuleIDs(pID))
            fModulesCal.push_back(*src.GetModuleCalibration(pID,mID));
    return *this;
}
        ...
        if (f != nullptr) {
            ...
            while ((key = (TKey*)nextkey())) {
                ...
                if (REST_Reflection::GetClassQuick(kName.c_str()) != nullptr &&
                    REST_Reflection::GetClassQuick(kName.c_str())->InheritsFrom("TRestCalibrationCorrection")) {
...
std::pair<int, int> TRestCalibrationCorrection::Module::GetIndexMatrix(const double x, const double y) const {
    ...
    if (fSplitX.upper_bound(x) != fSplitX.end()){
        ...
        if (index_x < 0){
            RESTWarning << "index_x < 0 for x = "<< x <<" and fSplitX[0]="<<*fSplitX.begin()<<p->RESTendl;
            index_x=0;
        }
    }
    else {RESTWarning << "x is out of split for x = "<< x <<p->RESTendl; index_x=fSplitX.size()-2;}

    if (fSplitY.upper_bound(y) != fSplitY.end()){
        index_y = std::distance(fSplitY.begin(), fSplitY.upper_bound(y)) -1;
        if (index_y < 0){
            RESTWarning << "index_y < 0 for y = "<< y <<" and fSplitY[0]="<<*fSplitY.begin()<<p->RESTendl;
            index_y=0;
        }
    }
    else {RESTWarning << "y is out of split for y = "<< y <<p->RESTendl; index_y=fSplitY.size()-2;}
...
void TRestCalibrationCorrection::Module::SetSplitX() {
    fSplitX.clear();
     for ( int i=0; i<=fNumberOfSegmentsX; i++) { // <= so that the last segment is included
        double x = fReadoutRange.X()+ ( (fReadoutRange.Y()-fReadoutRange.X())/(float)fNumberOfSegmentsX )*i;
        fSplitX.insert(x);
    }
}
/////////////////////////////////////////////
...
void TRestCalibrationCorrection::Module::SetSplitY() {
    fSplitY.clear();
     for ( int i=0; i<=fNumberOfSegmentsY; i++) { // <= so that the last segment is included
        double y = fReadoutRange.X()+ ( (fReadoutRange.Y()-fReadoutRange.X())/(float)fNumberOfSegmentsY )*i;
        fSplitY.insert(y);
    }
}
...
void TRestCalibrationCorrection::Module::CalculateCalibrationParameters() {
    ...
    if ( fCalibRange.X() >= fCalibRange.Y() ){
            ...
            if ( cut.empty()) cut = "1";
            auto histo = dataSet.GetDataFrame().Filter(cut).Histo1D(
            ...
            xMax = hpunt->GetXaxis()->GetXmax() * fraction/step; //previous step is the last one that nAtEndSpc<1%
            //Set the calibration range if needed
            //fCalibRange.SetX(xMin);
            fCalibRange.SetY(xMax);
            hpunt.reset(); //delete hpunt;
            RESTDebug << "Calibration range (auto)set to (" << fCalibRange.X() << "," << fCalibRange.Y() << ")" << p->RESTendl;
    }  
    
    //--- Definition of histogram matrix ---
    std::vector< std::vector <TH1F* > > h 
                (fNumberOfSegmentsX, std::vector<TH1F*>(fNumberOfSegmentsY, nullptr));
    for ( int i=0; i<h.size(); i++) {
         for ( int j=0; j<h.at(0).size(); j++) {
            h[i][j] = new TH1F("", "",  fNBins, fCalibRange.X(), fCalibRange.Y()); //h[column][row] equivalent to h[x][y]
         }
    }
    std::vector<std::vector <double> > calParSlope 
                (fNumberOfSegmentsX, std::vector<double> (fNumberOfSegmentsY,0));
    std::vector<std::vector <double> > calParIntercept 
                (fNumberOfSegmentsX, std::vector<double> (fNumberOfSegmentsY,0));
    ...
    for ( int i=0; i<h.size(); i++) {
        ...
        for ( int j=0; j<h.at(0).size(); j++) {
            ...
            if ( !fDefinitionCut.empty())
                segment_cut += "&&"+fDefinitionCut;
    ...
    for ( int i=0; i<h.size(); i++) {
         for ( int j=0; j<h.at(0).size(); j++) {
            ...
            const double ratio = peakPos.size()==0 ?
                        1 : peakPos.front() / fEnergyPeaks.front(); //to estimate peak position
            ...
            for (const auto& energy : fEnergyPeaks) {
                ...
                do {
                    if (fAutoRangePeaks){
                        if ( peakPos.size() > 0 ) {
                            ...
                            while ( !(start < pos && pos < end) ){ 
                                ...
                                pos = *std::next(std::find(peakPos.begin(),peakPos.end(), pos)); //get next peak position
                            }
                            peakPos.erase(std::find(peakPos.begin(),peakPos.end(), pos)); //remove this peak position from the list
                            // final estimation of the peak range (idem fitting window) with this peak position pos
                            start = pos * 0.8;
                            end = pos * 1.2;
                    ...
                    RESTExtreme << "\t\tat " << DoubleToString(pos, "%.3g")
                                << ". Range(" << DoubleToString(start, "%.3g") << ", " 
                                << DoubleToString(end, "%.3g") << ")"<< p->RESTendl;
...
///
void TRestCalibrationCorrection::Module::Refit(const double x, const double y, 
                                                       const double energyPeak, const TVector2& range) {
    auto [index_x, index_y] = GetIndexMatrix(x,y);
    int peakNumber = -1;
...
/// \param range The range for the fitting of the peak (in the observables corresponding units).
///
void TRestCalibrationCorrection::Module::Refit(const int x, const int y,
                                                       const int peakNumber,const TVector2& range){
...
void TRestCalibrationCorrection::Module::LoadConfigFromTiXmlElement(const TiXmlElement* module){
    ...
    while (peakDefinition != nullptr) {
        ...
        std::string ell = !peakDefinition->Attribute("energy") ? "Not defined" : peakDefinition->Attribute("energy") ;
...
void TRestCalibrationCorrection::Module::DrawSpectrum(const int index_x, const int index_y, TCanvas *c){
    ...
    if ( !c ){
        std::string t = "spectrum_"+std::to_string(fPlaneId)+"_"+std::to_string(fModuleId)
                        +"_"+std::to_string(index_x)+"_"+std::to_string(index_y);
...
void TRestCalibrationCorrection::Module::DrawFullSpectrum(){
    ...
    TH1F* sumHist = new TH1F("fullSpc", "", fSegSpectra[0][0]->GetNbinsX(),
                                            fSegSpectra[0][0]->GetXaxis()->GetXmin(),
...
void TRestCalibrationCorrection::Module::DrawLinearFit(const int index_x, const int index_y, TCanvas *c){
    ...
    if ( !c ){
        std::string t = "linearFit_"+std::to_string(fPlaneId)+"_"+std::to_string(fModuleId)
                        +"_"+std::to_string(index_x)+"_"+std::to_string(index_y);
        c = new TCanvas(t.c_str(),t.c_str());
    }
    auto xLower = *std::next(fSplitX.begin(), index_x);
...
void TRestCalibrationCorrection::Module::DrawGainMap(const int peakNumber){
    ...
    std::string t = "gainMap"+std::to_string(peakNumber)
                    +"_"+std::to_string(fPlaneId)+"_"+std::to_string(fModuleId);
    TCanvas *gainMap = new TCanvas(t.c_str(),t.c_str());   
    TH2F *hGainMap= new TH2F(("h"+t).c_str(), title.c_str(),
                             fNumberOfSegmentsY, fReadoutRange.X(), fReadoutRange.Y(),
                             fNumberOfSegmentsX, fReadoutRange.X(), fReadoutRange.Y()); 
...
void TRestCalibrationCorrection::Module::Print() {
    ...
    for (const auto& peak : fEnergyPeaks) RESTMetadata << peak << ", ";
        RESTMetadata << p->RESTendl;
    bool anyRange = false;
    for (const auto& r : fRangePeaks) if (r.X() < r.Y()){RESTMetadata<<" Range peaks: ";anyRange=true;break;} 
    if (anyRange) for (const auto& r : fRangePeaks) RESTMetadata << "(" << r.X() << ", " << r.Y() << ") ";
    ...
    RESTMetadata << " Calibration range: (" << fCalibRange.X() << ", " << fCalibRange.Y() << " )" << p->RESTendl;
    ...
    RESTMetadata << " Readout range (" << fReadoutRange.X() << ", " << fReadoutRange.Y() << " )" << p->RESTendl;
    RESTMetadata << "SplitX: ";
    for (auto &i : fSplitX){
        RESTMetadata << "    " << i;
    }
    ...
    int maxSize = 0;
    for (auto& x : fSlope) if (maxSize < x.size()) maxSize = x.size();
    ...
    maxSize = 0;
    for (auto& x : fIntercept) if (maxSize < x.size()) maxSize = x.size();
    for (int j = 0; j < maxSize; j++) {
