...
def update():
    ...
    if newgit == False:
        ...
        os.system("git stash")
        p = subprocess.Popen(
            ["git fetch origin " + vars.opt["Branch"] + " --tags"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            shell=True,
        )
...
if __name__ == "__main__":
    ...
    if vars.software["REST"] == True:
        print("")
        print(
            "!!!Updating REST from git repository!!! Target branch: "
            + vars.opt["Branch"]
        )
        print(
            "!!!Local changes will be overwritten!!! (except additions). Make backup and do it carefully."
        )
