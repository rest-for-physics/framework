...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.btn3 = Button(self,text="Launch Cluster Jobs",height=1,width=10)
        self.btn3.grid(row=0,column=3)

        self.label = Label(self,textvariable=self.var,height=3,width=60)
        self.label.grid(row=1,column=0,columnspan=6)
        self.var.set("select an item to install")

        self.t = Text(self,height=1,width=40)  
        self.t.grid(row=3,column=1,columnspan=4)
        self.t.insert("1.0","input options here")

        self.btn5 = Button(self,text="previous")
        self.btn5.config(state='disabled')
        self.btn5.grid(row=3,column=0)
        self.btn5["command"]=self.stepminus

        self.btn6 = Button(self,text="next")
        self.btn6.config(state='disabled')
        self.btn6.grid(row=3,column=5)
        self.btn6["command"]=self.stepplus


    var = StringVar()
    step = 0
    #(0~9 REST, 10~19 restG4, <0 : install completed, 0\10\20... : begin of wizard)
