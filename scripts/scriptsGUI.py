...
def refreshdisplay():
    ...
    if step == install_begin:
        ...
        instruction1.set(
            "Cmake flages: " + vars.cmakeflags[0] + " " + vars.cmakeflags[1]
        )
...
def nextstep():
    ...
    if step == install_begin:
        ...
        if "fatal" in err:
            step = install_git
        elif os.path.exists(vars.opt["Build_Path"]) and os.listdir(
            vars.opt["Build_Path"]
        ):
        ...
        installers.initgit()
        if os.path.exists(vars.opt["Build_Path"]) and os.listdir(
            vars.opt["Build_Path"]
        ):
            step = install_clearbuild
        else:
        ...
        if chVarDis1.get() == 1:
            ...
            os.environ["LD_LIBRARY_PATH"] = (
                vars.opt["Install_Path"] + "/lib:" + os.environ["LD_LIBRARY_PATH"]
            )
